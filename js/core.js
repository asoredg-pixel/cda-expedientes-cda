...
function medioNotificacionRespLabel(v){
  if(v==='electronica'||v==='correo')return 'Correo';
  if(v==='fisica'||v==='presencial')return 'Presencial / física';
  if(v==='whatsapp')return 'WhatsApp';
  if(v==='pagina')return 'Por página web';
  if(v==='aviso'||v==='avisos')return 'Por aviso';
  return medioNotificacionLabel(v)||v||'—';
}
function medioNotificacionRespFlagHtml(v){
  return medioNotificacionFlagHtml(v,false)||'';
}
/** Mapea el medio de radicación (secretaría) al canal de respuesta del workflow. */
function pqrsCanalDesdeMedioRadicacion(medio){
  const m=String(medio||'').trim().toLowerCase();
  if(m==='electronica'||m==='correo')return PQRS_WF_CANAL.CORREO;
  if(m==='fisica'||m==='presencial')return PQRS_WF_CANAL.PRESENCIAL;
  if(m==='whatsapp')return PQRS_WF_CANAL.WHATSAPP;
  if(m==='avisos'||m==='aviso')return PQRS_WF_CANAL.AVISO;
  return '';
}
/** Canal predeterminado al responder: workflow → medio radicación → correo si hay email → presencial. */
function pqrsCanalDefaultRespuesta(e){
  if(!e)return PQRS_WF_CANAL.CORREO;
  const wf=getPqrsWorkflow(e);
  if(wf.canal)return wf.canal;
  if(e._pqrs_respuesta_medio){
    const m=pqrsCanalDesdeMedioRadicacion(e._pqrs_respuesta_medio)||e._pqrs_respuesta_medio;
    if(m===PQRS_WF_CANAL.CORREO||m===PQRS_WF_CANAL.WHATSAPP||m===PQRS_WF_CANAL.PRESENCIAL||m===PQRS_WF_CANAL.AVISO||m===PQRS_WF_CANAL.FISICA)return m===PQRS_WF_CANAL.FISICA?PQRS_WF_CANAL.PRESENCIAL:m;
  }
  const desdeRad=pqrsCanalDesdeMedioRadicacion(e._medio_notificacion);
  if(desdeRad)return desdeRad;
  const correos=typeof pqrsCorreosCiudadano==='function'?pqrsCorreosCiudadano(e):[];
  return correos.length?PQRS_WF_CANAL.CORREO:PQRS_WF_CANAL.PRESENCIAL;
}
function pqrsEsCanalCorreo(canal){
  return String(canal||'')===PQRS_WF_CANAL.CORREO||String(canal||'')==='electronica';
}
function pqrsEsCanalNotifPersonal(canal){
  const c=String(canal||'');
  return c===PQRS_WF_CANAL.PRESENCIAL||c===PQRS_WF_CANAL.FISICA||c===PQRS_WF_CANAL.WHATSAPP||c===PQRS_WF_CANAL.AVISO||c==='aviso';
}
...
function pqrsEnParaFirma(e){
  ...
  return f===PQRS_WF.PARA_FIRMA||f===PQRS_WF.VITAL_GESTION;
}
...
function pqrsNotifQuienLabelUi(quien){
  ...
  if(typeof esModoResponsable==='function'&&esModoResponsable()&&responsableActivo
    &&typeof agendaNorm==='function'&&agendaNorm(q)===agendaNorm(responsableActivo))return'';
  return q;
}
...
function pqrsPuedeFlujoPorImprimir(){
  ...
  // Admin global ayudando a NCA, no cuando opera como oficina DEGUV/Secretaría
  if(typeof esAdministrador==='function'&&esAdministrador()
    &&!(typeof esModoOficinaDeguv==='function'&&esModoOficinaDeguv())
    &&!(typeof esSecretaria==='function'&&esSecretaria()))return true;
  return false;
}
/** Bandeja «Por firmar» / atajo a Director: NCA, VITAL, Director y oficinas (RN/OAP/Admin/Secretaría). */
function pqrsPuedeFlujoPorFirmarBandeja(){
  if(pqrsPuedeFlujoPorImprimir())return true;
...
function pqrsPuedeFirmarDirector(e){
  ...
  return !!(typeof esDirectorDsDeguv==='function'&&esDirectorDsDeguv()||esAdministrador());
}
/** VITAL / NCA / oficinas / encargado: roles que gestionan firma física sin ser el Director. */
function pqrsEsRolGestionFirmaSinDirector(){
  if(typeof esCargoVital==='function'&&esCargoVital())return true;
  if(typeof esNcaDeguv==='function'&&esNcaDeguv())return true;
  if(typeof esOficinaPqrsNca==='function'&&esOficinaPqrsNca())return true;
  if(typeof esAdministrador==='function'&&esAdministrador())return true;
...
function pqrsNombresVital(){
  ...
  (typeof _usuariosCache!=='undefined'?_usuariosCache:[]).forEach(function(u){
    ...
    if(String(u.cargo||'').toLowerCase()!=='vital')return;
...
function pqrsEsNotificadorAutorizadoCorreo(nombre,ofi){
  const n=String(nombre||'').trim();
  if(!n)return false;
  return pqrsNotificadoresAutorizadosCorreo(ofi).some(function(x){
...
function pqrsAplicarReglaNotificadorCanal(e,canal,notificarPor){
  const prefer=String(notificarPor||'').trim();
  if(!e)return prefer;
  const ofi=String(e._pqrs_oficina||'guaviare').trim()||'guaviare';
...
function pqrsPuedeNotificarOficio(e){
  ...
  // Sin designación: cualquier responsable asignado a la actividad Atender PQRSD (flujo actual)
  if(esModoResponsable()&&responsableActivo){
    ...
    if(t&&typeof taskUsuarioEsAsignado==='function'&&taskUsuarioEsAsignado(t,responsableActivo))return true;
  }
  return false;
}
/** Tras aprobación NCA (firma o mensaje): la proyección del responsable ya cumplió. */
 ...
 * excepto quien esté designado para notificar (sigue abierta hasta notificar).
...
function pqrsSincronizarParticipacionPostAprobacion(e){
  ...
  (t.responsables||[]).forEach(n=>{if(n)nombres.add(String(n).trim());});
  if(wf.entregado_por)nombres.add(String(wf.entregado_por).trim());
  nombres.forEach(function(n){
    if(!n)return;
    const a=ensureAsignado(t,n);
    if(!a)return;
    const esNotif=enNotif&&notifPor&&agendaNorm(notifPor)===agendaNorm(n);
    if(esNotif){
      a.fechaAtendida='';
  ...
  if(fase===PQRS_WF.CERRADA||(typeof pqrsEstaCerrada==='function'&&pqrsEstaCerrada(e))){
    syncTaskAggregateState(t);
    return;
  }
  // Tarea agregada: no «cerrar» del todo mientras falte notificar
  if(enNotif&&notifPor){
    const aN=getAsignado(t,notifPor);
    t.fechaAtendida='';
    t.fechaReportada=t.fechaReportada||(aN&&aN.fechaReportada)||hoyStr;
    t.estado=(aN&&aN.fechaReportada)?'Por verificar':'En ejecución';
  }else if((t.asignados||[]).length<=1){
    const a=(t.asignados||[])[0];
    if(a&&a.estado==='atendido'){
      t.fechaReportada=a.fechaReportada||hoyStr;
      t.fechaAtendida=a.fechaAtendida||hoyStr;
      t.estado='Atendida';
    }
  }else{
    syncTaskAggregateState(t);
    if(t.estado==='Atendida'){/* proyección lista; flujo firma/notif sigue vía fase */}
  }
  if(!Array.isArray(t.historial))t.historial=[];
  if(!t.historial.some(h=>h&&h.tipo==='pqrs_proyeccion_atendida')){
...
function openPqrsSidePanel(expId){
  ...
  if(!expId)return;
  // Si es fila de trámite en firma, abrir modal de firma (no panel PQRSD)
  const eQuick=typeof getExpById==='function'?getExpById(expId):null;
  if(eQuick&&!(typeof esPqrsSecretaria==='function'&&esPqrsSecretaria(eQuick))){
    const tFirma=(eQuick.tasks||[]).find(function(tk){
    ...
    if(tFirma&&typeof openTramiteDirectorFirmarModal==='function'){
      openTramiteDirectorFirmarModal(expId,tFirma.id);
      return;
    }
  }
  ...
  if(fase!==PQRS_WF.SIN_RESPUESTA&&fase!==PQRS_WF.CERRADA){
    ...
    if(fase===PQRS_WF.POR_FIRMAR&&(pqrsPuedeFirmarDirector(e)||(typeof pqrsPuedeMarcarFirmadoSinCargar==='function'&&pqrsPuedeMarcarFirmadoSinCargar(e))||(typeof pqrsPuedeAsignarPorNotificar==='function'&&pqrsPuedeAsignarPorNotificar(e)))){
      if(typeof esDirectorDsDeguv==='function'&&esDirectorDsDeguv())
        acciones+='<button type="button" class="btn bsm" style="background:#185FA5;color:#fff;margin-top:6px" onclick="openPqrsDirectorFirmarModal(\''+id+'\',\'ver\')">👁 Ver documento</button> ';
      else
        acciones+='<button type="button" class="btn bsm" style="background:#15803d;color:#fff;margin-top:6px" onclick="openPqrsDirectorFirmarModal(\''+id+'\')">'+(typeof pqrsPuedeFirmarDirector==='function'&&pqrsPuedeFirmarDirector(e)?'🖊 Firmar / cargar PDF':'✓ Ya firmado / notificar')+'</button> ';
    }
    if((fase===PQRS_WF.PENDIENTE_NOTIF||fase===PQRS_WF.LISTA_ENVIO)&&pqrsPuedeNotificarOficio(e))
      acciones+='<button type="button" class="btn bsm bp" style="margin-top:6px" onclick="openPqrsNotificarOficioModal(\''+id+'\')">📬 Notificar</button> ';
...
async function openPqrsRespuestaModal(expId,opts){
  ...
  // Responder → «Oficio firmado» (notificar). Botón 🖊 → modoFirma («Oficio para firma»).
  const tipOficioLbl=modoFirma?'📄 Oficio para firma':'📄 Oficio firmado';
  ...
  body.innerHTML=pqrsSelHtml+
    ...
    '</div>'):'')+
    '<div class="fld" id="pqrs-resp-canal-wrap" style="margin-bottom:10px"><label style="font-weight:600;font-size:12px">Canal de notificación</label>'+
    ...
    '</div><input type="hidden" id="pqrs-resp-canal" value="'+escAttr(canalDef)+'"></div>'+
    (fromGmail?'':(
    ...
    '<div id="pqrs-compose-att-err" class="fld-err" style="display:none"></div>'+
    ...
    '</div></div></div>'))+
    '<div class="fld" id="pqrs-resp-cuerpo-wrap" style="margin-bottom:10px;display:none"><label style="font-weight:600;font-size:12px" id="pqrs-resp-cuerpo-label">Descripción</label>'+
    '<textarea id="pqrs-resp-cuerpo" placeholder="" style="min-height:120px;padding:8px;border:1px solid var(--bd);border-radius:var(--r);font-size:13px;font-family:\'DM Sans\',sans-serif;width:100%;margin-top:5px;box-sizing:border-box;line-height:1.45">'+escAttr(cuerpoVal)+'</textarea></div>'+
    '<div class="fld" id="pqrs-resp-adj-wrap" style="margin-bottom:10px"><label style="font-weight:600;font-size:12px" id="pqrs-resp-adj-label">Documento</label>'+
    '<div id="pqrs-resp-adj-hint" style="display:none"></div>'+
    '<div id="pqrs-resp-adj-rows" style="margin-top:4px"></div>'+
    '<div id="pqrs-resp-adj-err" class="fld-err" style="display:none"></div>'+
    '<div class="fx" style="gap:8px;flex-wrap:wrap;align-items:center;margin-top:4px">'+
    (usaDriveInst?'<button type="button" class="btn bsm" id="pqrs-resp-adj-btn" onclick="addPqrsRespAdjFile()">📎 Adjuntar archivo</button>':'')+
    '<label id="pqrs-resp-anexos-wrap" class="fx" style="display:none;gap:6px;align-items:center;font-size:12px;cursor:pointer;margin:0">'+
      '<input type="checkbox" id="pqrs-resp-tiene-anexos" onchange="pqrsRespToggleAnexos()">'+
      '<span>Incluye anexos</span>'+
    '</label>'+
    '</div>'+
    '<div id="pqrs-resp-anexos-box" style="display:none;margin-top:6px">'+
    ...
    '</div>'+
    '</div>'+
    '<div class="fld" id="pqrs-resp-nota-wrap" style="margin-bottom:10px"><label>Notas internas</label>'+
    '<textarea id="pqrs-resp-nota" placeholder="Opcional…" style="min-height:44px;padding:6px;border:1px solid var(--bd);border-radius:var(--r);font-size:11px;font-family:\'DM Sans\',sans-serif;width:100%;margin-top:4px">'+escAttr((e&&e._pqrs_notas_internas)||'')+'</textarea></div>'+
    '<div class="fx" style="gap:8px;flex-wrap:wrap">'+
    (fromGmail?'':
    '<button type="button" class="btn bsm" id="pqrs-resp-firma-btn" style="display:none;background:#0d5c2e;color:#fff;border-color:#0d5c2e" onclick="submitPqrsRespuestaParaFirma(\''+escAttr(expLabel)+'\')">🖊 Enviar a firma del Director</button>')+
    (fromGmail?'':
  ...
  setPqrsRespCanal(canalDef);
  // Solo mensaje simple lleva plantilla aquí; oficio: plantilla al notificar
  if(tipoIni===PQRS_WF_TIPO.MENSAJE)pqrsAplicarPlantillaSegunTipo(tipoIni,false);
...
function pqrsClearRespOficioError(){
  ...
  const inp=document.getElementById('pqrs-resp-oficio');
  if(el){el.style.display='none';el.textContent='';}
  if(inp)inp.classList.remove('fld-invalid');
}
function pqrsClearRespComposeAttError(){
  const el=document.getElementById('pqrs-compose-att-err');
  if(el){el.style.display='none';el.textContent='';}
}
function pqrsClearRespAdjError(){
  const el=document.getElementById('pqrs-resp-adj-err');
  if(el){el.style.display='none';el.textContent='';}
}
function pqrsShowRespOficioError(msg){
  const el=document.getElementById('pqrs-resp-oficio-err');
  const inp=document.getElementById('pqrs-resp-oficio');
  if(inp)inp.classList.add('fld-invalid');
  if(el){el.style.display='';el.textContent=msg||'Indique el N° de oficio.';}
  if(inp)inp.focus();
}
function pqrsShowRespComposeAttError(msg){
  const el=document.getElementById('pqrs-compose-att-err');
  if(el){el.style.display='';el.textContent=msg||'Adjunte el oficio firmado.';}
}
function pqrsShowRespAdjError(msg){
  const el=document.getElementById('pqrs-resp-adj-err');
  if(el){el.style.display='';el.textContent=msg||'Adjunte los documentos requeridos.';}
}
function pqrsCountAdjuntosRespuesta(){
  const adj=collectPqrsRespAdjuntos();
  const compose=(window._pqrsComposeAttachments||[]).length;
  return(adj.files?adj.files.length:0)+(adj.links?adj.links.length:0)+compose;
}
function pqrsValidateOficioRespuesta(tipoResp,opts){
  opts=opts||{};
  pqrsClearRespOficioError();
  pqrsClearRespComposeAttError();
  pqrsClearRespAdjError();
  if(tipoResp!==PQRS_WF_TIPO.OFICIO)return true;
  const oficio=String((document.getElementById('pqrs-resp-oficio')||{}).value||'').trim();
  if(!oficio){pqrsShowRespOficioError('Favor diligenciar el N° de oficio.');return false;}
  if(opts.requireComposeAtt){
    const files=window._pqrsComposeAttachments||[];
    if(!files.length){pqrsShowRespComposeAttError('Para oficio firmado debe adjuntar el documento del oficio.');return false;}
  }
  if(opts.requireAdj){
    if(pqrsCountAdjuntosRespuesta()<1){
      pqrsShowRespAdjError(opts.adjMsg||'Para oficio firmado debe adjuntar el documento del oficio.');
      return false;
    }
  }
...
function pqrsValidateAdjuntosPorCanal(tipoResp,canal,opts){
  ...
  pqrsClearRespAdjError();
  pqrsClearRespComposeAttError();
  const n=pqrsCountAdjuntosRespuesta();
  if(tipoResp===PQRS_WF_TIPO.OFICIO&&pqrsEsCanalCorreo(canal)){
    // En correo el adjunto va en compose
    return true;
  }
  if(tipoResp===PQRS_WF_TIPO.OFICIO&&!pqrsEsCanalCorreo(canal)){
    if(n<1){pqrsShowRespAdjError('Adjunte el oficio firmado.');return false;}
  }
  if(canal===PQRS_WF_CANAL.WHATSAPP){
    // Oficio (si aplica) + soporte de notificación (img/pdf)
    const min=tipoResp===PQRS_WF_TIPO.OFICIO?2:1;
    if(n<min){
      pqrsShowRespAdjError(tipoResp===PQRS_WF_TIPO.OFICIO
        ?'Adjunte el oficio firmado y el soporte de notificación por WhatsApp (imagen o PDF).'
        :'Adjunte el soporte de notificación por WhatsApp.');
      return false;
    }
  }
  if(canal===PQRS_WF_CANAL.AVISO){
    const min=tipoResp===PQRS_WF_TIPO.OFICIO?2:1;
    if(n<min){
      pqrsShowRespAdjError(tipoResp===PQRS_WF_TIPO.OFICIO
        ?'Adjunte el oficio y el soporte de notificación por aviso.'
        :'Adjunte el soporte de notificación por aviso.');
      return false;
    }
  }
  if((canal===PQRS_WF_CANAL.PRESENCIAL||canal===PQRS_WF_CANAL.FISICA)&&tipoResp===PQRS_WF_TIPO.OFICIO){
    if(n<1){pqrsShowRespAdjError('Adjunte el oficio firmado (y constancia de notificación si la tiene).');return false;}
  }
  return true;
}
...
function pqrsAplicarPlantillaSegunTipo(tipo,force){
  ...
  // Oficio: plantilla solo al notificar (VITAL / NCA / oficina en Por notificar).
  // Mensaje: plantilla en compose / entrega.
  const ids=tipo===PQRS_WF_TIPO.OFICIO
    ?['pqrs-notif-cuerpo']
    :['pqrs-resp-cuerpo','pqrs-compose-body','pqrs-entrega-resp-cuerpo','pqrs-notif-cuerpo'];
  ...
  // Al pasar a oficio en Responder, limpiar plantilla del compose (no se usa hasta notificar)
  if(tipo===PQRS_WF_TIPO.OFICIO&&(force||true)){
    ['pqrs-compose-body','pqrs-resp-cuerpo'].forEach(function(id){
      const el=document.getElementById(id);
      if(el&&_pqrsEsPlantillaRespuesta(el.value))el.value='';
    });
  }
}
...
function pqrsRespRefreshModalUi(){
  // Formulario de entrega (responsable): no alterar UI del modal de respuesta
  if(document.getElementById('pqrs-entrega-resp-cuerpo')&&!document.getElementById('pqrs-resp-email-compose')&&!document.getElementById('pqrs-resp-cuerpo')){
    return;
  }
  const tipo=String((document.getElementById('pqrs-resp-tipo')||{}).value||PQRS_WF_TIPO.MENSAJE);
  const canal=String((document.getElementById('pqrs-resp-canal')||{}).value||PQRS_WF_CANAL.CORREO);
  const isInfo=tipo===PQRS_WF_TIPO.INFORMATIVA;
  const isOficio=tipo===PQRS_WF_TIPO.OFICIO;
  const isMensaje=tipo===PQRS_WF_TIPO.MENSAJE;
  const isCorreo=pqrsEsCanalCorreo(canal);
  const isPersonal=pqrsEsCanalNotifPersonal(canal);
  const modoFirmaCtx=!!(window._taskModalCtx&&window._taskModalCtx.modoFirma);
  // Mensaje simple solo por correo; si canal no es correo, pasar a oficio
  if(!isInfo&&isMensaje&&!isCorreo){
    const hid=document.getElementById('pqrs-resp-tipo');
    if(hid)hid.value=PQRS_WF_TIPO.OFICIO;
    document.querySelectorAll('#pqrs-resp-tipo-btns .tipo-resp-btn').forEach(b=>{
      b.classList.toggle('on',b.getAttribute('data-val')===PQRS_WF_TIPO.OFICIO);
    });
    return pqrsRespRefreshModalUi();
  }
  // modoFirma (botón 🖊): oficio → Director. Responder + oficio: ya firmado → notificar.
  const useEmail=!isInfo&&isCorreo&&isMensaje;
  const puedeFirma=modoFirmaCtx&&isOficio&&!isInfo&&typeof pqrsPuedeAtajoParaFirma==='function'&&pqrsPuedeAtajoParaFirma();
  const oficioNotificar=isOficio&&!isInfo&&!modoFirmaCtx;
  const compose=document.getElementById('pqrs-resp-email-compose');
  const cuerpoWrap=document.getElementById('pqrs-resp-cuerpo-wrap');
  const canalWrap=document.getElementById('pqrs-resp-canal-wrap');
  const adjWrap=document.getElementById('pqrs-resp-adj-wrap');
  const anexosWrap=document.getElementById('pqrs-resp-anexos-wrap');
  const emailBtn=document.getElementById('pqrs-resp-email-send-btn');
  const submitBtn=document.getElementById('pqrs-resp-submit-btn');
  const firmaBtn=document.getElementById('pqrs-resp-firma-btn');
  const oficioReq=document.getElementById('pqrs-resp-oficio-req');
  const oficioWrap=document.getElementById('pqrs-resp-oficio-wrap');
  const fechaLbl=document.getElementById('pqrs-resp-fecha-label');
  const adjLabel=document.getElementById('pqrs-resp-adj-label');
  const adjHint=document.getElementById('pqrs-resp-adj-hint');
  const cuerpoLbl=document.getElementById('pqrs-resp-cuerpo-label');

  if(canalWrap)canalWrap.style.display=isInfo||modoFirmaCtx?'none':'';
  document.querySelectorAll('#pqrs-resp-canal-btns .canal-resp-btn').forEach(function(b){
    b.style.display=(isInfo||modoFirmaCtx)?'none':'';
    b.classList.toggle('on',b.getAttribute('data-val')===canal);
  });
  document.querySelectorAll('#pqrs-resp-tipo-btns .tipo-resp-btn').forEach(function(b){
    const v=b.getAttribute('data-val');
    if(v===PQRS_WF_TIPO.MENSAJE)b.style.display=(!isInfo&&!isCorreo)||modoFirmaCtx?'none':'';
    if(v===PQRS_WF_TIPO.INFORMATIVA&&modoFirmaCtx)b.style.display='none';
  });

  if(compose)compose.style.display=(useEmail||(oficioNotificar&&isCorreo))?'':'none';
  const showCuerpo=isInfo||(!useEmail&&!isInfo&&oficioNotificar&&isPersonal);
  if(cuerpoWrap)cuerpoWrap.style.display=showCuerpo?'':'none';
  if(cuerpoLbl){
    if(isInfo)cuerpoLbl.innerHTML='Descripción <span class="req-star">*</span>';
    else if(isPersonal)cuerpoLbl.textContent='Nota (opcional)';
    else cuerpoLbl.textContent='Descripción';
  }
  const cuerpoTxt=document.getElementById('pqrs-resp-cuerpo');
  if(cuerpoTxt){
    if(isInfo){
      cuerpoTxt.placeholder='Descripción visible en consulta ciudadana…';
      if(_pqrsEsPlantillaRespuesta(cuerpoTxt.value))cuerpoTxt.value='';
    }else if(isOficio&&_pqrsEsPlantillaRespuesta(cuerpoTxt.value)){
      cuerpoTxt.value='';
    }else if(!cuerpoTxt.value){
      cuerpoTxt.placeholder=isPersonal?'Opcional…':'';
    }
  }
  const composeBody=document.getElementById('pqrs-compose-body');
  if(composeBody&&isOficio&&_pqrsEsPlantillaRespuesta(composeBody.value))composeBody.value='';

  // Oficio: documento + anexos. Mensaje: adjuntos en compose.
  if(adjWrap)adjWrap.style.display=(useEmail||isInfo)?'none':'';
  if(anexosWrap)anexosWrap.style.display=isOficio&&!isInfo?'inline-flex':'none';
  if(!isOficio){
    const cb=document.getElementById('pqrs-resp-tiene-anexos');
    if(cb&&cb.checked){cb.checked=false;pqrsRespToggleAnexos();}
  }
  if(adjLabel){
    if(puedeFirma)adjLabel.textContent='Oficio para firma (PDF)';
    else if(oficioNotificar)adjLabel.textContent='Oficio firmado (PDF)';
    else if(canal===PQRS_WF_CANAL.WHATSAPP)adjLabel.textContent='Oficio y soporte WhatsApp';
    else if(canal===PQRS_WF_CANAL.AVISO)adjLabel.textContent='Oficio y soporte por aviso';
    else if(isPersonal)adjLabel.textContent='Oficio / constancia';
    else adjLabel.textContent='Documento';
  }
  if(adjHint){adjHint.style.display='none';adjHint.textContent='';}

  if(fechaLbl){
    fechaLbl.innerHTML=(isPersonal||(oficioNotificar&&!isCorreo))
      ?'Fecha de notificación<span class="req-star">*</span>'
      :'Fecha de la respuesta<span class="req-star">*</span>';
  }

  if(firmaBtn){
    firmaBtn.style.display=puedeFirma?'':'none';
    firmaBtn.textContent='🖊 Enviar a firma del Director';
  }
  if(emailBtn){
    emailBtn.style.display=(useEmail||(oficioNotificar&&isCorreo))?'':'none';
    emailBtn.textContent='📤 Enviar correo y cerrar';
  }
  if(submitBtn){
    const hideSubmit=useEmail||(puedeFirma)||(oficioNotificar&&isCorreo);
    submitBtn.style.display=hideSubmit?'none':'';
    if(isInfo)submitBtn.textContent='ℹ️ Marcar informativa y cerrar';
    else if(oficioNotificar||isPersonal)submitBtn.textContent='✅ Registrar notificación y cerrar';
    else submitBtn.textContent='✅ Confirmar y cerrar';
  }
  if(oficioWrap)oficioWrap.style.display=isInfo?'none':(isMensaje?'none':'');
  if(oficioReq)oficioReq.style.display=isOficio?'':'none';

  if(useEmail)pqrsAplicarPlantillaSegunTipo(PQRS_WF_TIPO.MENSAJE,false);
  else if(isPersonal||oficioNotificar){
    const cuerpoEl=document.getElementById('pqrs-resp-cuerpo');
    if(cuerpoEl&&_pqrsEsPlantillaRespuesta(cuerpoEl.value))cuerpoEl.value='';
  }
}
...
function pqrsComposeRenderAttachments(){
  ...
  if(typeof pqrsClearRespComposeAttError==='function')pqrsClearRespComposeAttError();
...
function setPqrsRespTipo(val){
  ...
  if(val===PQRS_WF_TIPO.MENSAJE&&!window._gmailVinculoMsg){
    const canHid=document.getElementById('pqrs-resp-canal');
    if(canHid&&!pqrsEsCanalCorreo(canHid.value)){
      canHid.value=PQRS_WF_CANAL.CORREO;
      document.querySelectorAll('#pqrs-resp-canal-btns .canal-resp-btn').forEach(b=>{
        b.classList.toggle('on',b.getAttribute('data-val')===PQRS_WF_CANAL.CORREO);
      });
    }
  }
  if(val===PQRS_WF_TIPO.MENSAJE){
...
function setPqrsRespCanal(val){
  ...
  if(!pqrsEsCanalCorreo(val)){
    const tipoHid=document.getElementById('pqrs-resp-tipo');
    if(tipoHid&&tipoHid.value===PQRS_WF_TIPO.MENSAJE){
      tipoHid.value=PQRS_WF_TIPO.OFICIO;
      document.querySelectorAll('#pqrs-resp-tipo-btns .tipo-resp-btn').forEach(b=>{
        b.classList.toggle('on',b.getAttribute('data-val')===PQRS_WF_TIPO.OFICIO);
      });
    }
...
function onPqrsRespOficioInput(){
  ...
  const canal=String((document.getElementById('pqrs-resp-canal')||{}).value||'');
  if(!pqrsEsCanalCorreo(canal))return;
  ...
  (typeof sstSolicitarGmailParaAdjuntar==='function'?sstSolicitarGmailParaAdjuntar():Promise.resolve(true)).then(function(ok){
    ...
    inp.onchange=function(){
      ...
      Array.from(inp.files||[]).forEach(f=>{
        ...
        row._adjFile=f;
        box.appendChild(row);
        if(typeof pqrsClearRespAdjError==='function')pqrsClearRespAdjError();
      });
    };
    inp.click();
  });
...
function addPqrsRespAdjRow(boxId){
  // Legacy: ya no se usa «+ Link Drive» en Responder; se conserva por compatibilidad
...
function pqrsRespToggleAnexos(){
  const cb=document.getElementById('pqrs-resp-tiene-anexos');
  const box=document.getElementById('pqrs-resp-anexos-box');
  if(!box)return;
  const on=!!(cb&&cb.checked);
  box.style.display=on?'':'none';
  if(!on)pqrsRespClearAnexos();
}
function pqrsRespAddAnexos(){
  (typeof sstSolicitarGmailParaAdjuntar==='function'?sstSolicitarGmailParaAdjuntar():Promise.resolve(true)).then(function(ok){
    if(!ok)return;
    const inp=document.getElementById('pqrs-resp-anexos-file');
    if(inp)inp.click();
  });
}
function pqrsRespOnAnexosChange(inp){
  const box=document.getElementById('pqrs-resp-anexos-list');
  if(!box)return;
  const files=inp&&inp.files?Array.from(inp.files):[];
  if(!files.length){box.innerHTML='';return;}
  box.innerHTML=files.map(function(f,i){
    return '<div class="fx" style="gap:6px;align-items:center;margin-top:4px;font-size:12px;padding:4px 6px;background:var(--sf2);border-radius:var(--r)">'+
      '📎 <span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><strong>Anexo '+(i+1)+'</strong> · '+escAttr(f.name)+'</span>'+
      (i===0?'<button type="button" class="btn bsm bd2" onclick="pqrsRespClearAnexos()">✕</button>':'')+
      '</div>';
  }).join('');
}
function pqrsRespClearAnexos(){
  const inp=document.getElementById('pqrs-resp-anexos-file');
  if(inp)inp.value='';
  const box=document.getElementById('pqrs-resp-anexos-list');
  ...
  const cb=document.getElementById('pqrs-resp-tiene-anexos');
  if(cb&&!cb.checked){
    const wrap=document.getElementById('pqrs-resp-anexos-box');
    if(wrap)wrap.style.display='none';
  }
}
function collectPqrsRespAnexos(){
  const inp=document.getElementById('pqrs-resp-anexos-file');
  const tiene=!!(document.getElementById('pqrs-resp-tiene-anexos')||{}).checked;
  if(!tiene||!inp||!inp.files||!inp.files.length)return[];
  return Array.from(inp.files).filter(Boolean);
}
function collectPqrsRespAdjuntos(boxId){
  ...
  return{links,files,anexos:collectPqrsRespAnexos()};
}
/**
 * Oficinas (RN/OAP/Admin/Secretaría/NCA): oficio + anexos → «Por firmar» del Director
 * (como NCA). Sin plantilla de correo aquí; la plantilla aparece al notificar.
 * Mensaje simple = atajo enviar correo y cerrar (submitPqrsRespuestaPorCorreo).
 */
async function submitPqrsRespuestaParaFirma(expId){
  ...
  if(!(typeof pqrsPuedeAtajoParaFirma==='function'&&pqrsPuedeAtajoParaFirma())){
    notif('No puede enviar a firma del Director desde esta sesión','err');return;
  }
  ...
  const canal=String((document.getElementById('pqrs-resp-canal')||{}).value||PQRS_WF_CANAL.CORREO).trim();
  const cuerpoTxt=String((document.getElementById('pqrs-resp-cuerpo')||{}).value||'').trim();
  const cuerpo=(!_pqrsEsPlantillaRespuesta(cuerpoTxt)?cuerpoTxt:'');
  const notaInterna=String((document.getElementById('pqrs-resp-nota')||{}).value||'').trim();
  if(tipoResp!==PQRS_WF_TIPO.OFICIO){notif('Seleccione «Oficio para firma»','err');return;}
  if(!fechaResp){notif('Indique la fecha de la respuesta','err');return;}
  if(!pqrsValidateOficioRespuesta(tipoResp,{requireAdj:true,adjMsg:'Adjunte el PDF del oficio',skipDupCheck:false}))return;

  const adj=collectPqrsRespAdjuntos();
  const oficioFiles=(adj.files||[]).map(function(x){return x&&x.file;}).filter(Boolean);
  const anexosFiles=adj.anexos||[];
  const tieneAnexos=!!(document.getElementById('pqrs-resp-tiene-anexos')||{}).checked;
  if(tieneAnexos&&!anexosFiles.length){
    notif('Marcó que hay anexos: adjunte al menos un archivo de anexo','err');return;
  }
  if(!oficioFiles.length){
    notif('Adjunte el PDF del oficio para firma del Director','err');return;
  }

  const firmaBtn=document.getElementById('pqrs-resp-firma-btn');
  const emailBtn=document.getElementById('pqrs-resp-email-send-btn');
  const submitBtn=document.getElementById('pqrs-resp-submit-btn');
  if(firmaBtn){firmaBtn.disabled=true;firmaBtn.textContent='Subiendo…';}
  ...
  try{
    if(!usaDriveInst){
      notif('Se requiere Drive institucional para enviar a firma','err');
      throw new Error('Sin Drive institucional');
    }
    ...
    if(!driveOk){
      if(firmaBtn){firmaBtn.disabled=false;firmaBtn.textContent='🖊 Enviar a firma del Director';}
      if(emailBtn)emailBtn.disabled=false;
      if(submitBtn)submitBtn.disabled=false;
      return;
    }
    const allUp=oficioFiles.map(function(f){return{file:f,esAnexo:false};})
      .concat(anexosFiles.map(function(f){return{file:f,esAnexo:true};}));
    if(typeof sstCargaShow==='function')sstCargaShow({title:'Enviando a firma',message:'Subiendo oficio y anexos al Drive…',pct:10,sub:oficioFiles[0].name||''});
    let anexoSeq=0;
    for(let i=0;i<allUp.length;i++){
      const item=allUp[i];
      const file=item.file;
      const esAnexo=!!item.esAnexo;
      if(esAnexo)anexoSeq++;
      const pref=esAnexo
      ...
      const res=await driveUploadInstitutional(file,pref,file.type||'application/pdf','respuesta_aprobada',expId,nombreCarpeta,e._fecha||e._fecha_solicitud||'',{expediente:e,uploadTarget:'respuesta'});
      if(esAnexo){
        documentos.push({
          nombre:'Anexo '+anexoSeq,
          labelAnexo:'Anexo '+anexoSeq,
          driveLink:res.driveLink,
          previewLink:res.previewLink||res.driveLink,
          fileId:res.fileId||'',
          tipo:'anexo_respuesta',
          es_anexo:true,
          anexo_n:anexoSeq,
          mime:file.type||'',
    ...
    if(!documentos.length){
      notif('No se pudo preparar el documento para firma','err');
    ...
    const patchWf={
      ...
      canal:canal,
      cuerpo:cuerpo||('Oficio '+oficioExt+' enviado a firma'),
      oficio:oficioExt,
      ...
      listo_firma:{por:por,en:new Date().toISOString(),atajo_digital:true},
      impreso:null,
      firma_fisica:null,
      firma_director:null
    };
    ...
    if(typeof _pqrsRenombrarDocsDriveWf==='function'){
      try{await _pqrsRenombrarDocsDriveWf(Object.assign({},wf,{documentos:docsFinal}),'por_firmar');}catch(errR){console.warn('submitPqrsRespuestaParaFirma rename:',errR);}
    }
    setPqrsWorkflow(e,patchWf);
    if(notaInterna)e._pqrs_notas_internas=notaInterna;
    e._pqrs_respuesta_oficio=oficioExt;
    e._pqrs_respuesta_fecha=fechaResp;
    e._pqrs_respuesta_nota=patchWf.cuerpo;
    if(!Array.isArray(e._pqrs_historial))e._pqrs_historial=[];
    e._pqrs_historial.push({
      tipo:'oficina_atajo_firma',
      fecha:hoy(),
      nota:'Oficio enviado a firma del Director'+(anexoSeq?' · '+anexoSeq+' anexo(s)':'')+(notifPor?' · Notificará: '+notifPor:''),
    ...
    refreshPqrsDetalleViews(expId);
    notif('🖊 Oficio en «Por firmar» — el Director lo verá en su bandeja'+(anexoSeq?' · '+anexoSeq+' anexo(s)':'')+(notifPor?' · Notificará: '+notifPor:''),'ok');
  }catch(err){
    if(typeof sstCargaHide==='function')sstCargaHide();
    console.error('submitPqrsRespuestaParaFirma:',err);
    notif('No se pudo enviar a firma: '+String(err.message||err).slice(0,90),'err');
    if(firmaBtn){firmaBtn.disabled=false;firmaBtn.textContent='🖊 Enviar a firma del Director';}
    if(emailBtn)emailBtn.disabled=false;
    if(submitBtn)submitBtn.disabled=false;
  }
...
async function submitPqrsRespuesta(expId){
  ...
  const adj=collectPqrsRespAdjuntos();
  const isPersonal=pqrsEsCanalNotifPersonal(canal);
  if(!fechaResp){notif(isPersonal?'Indique la fecha de notificación':'Indique la fecha de la respuesta','err');return;}
    ...
    for(const {file,statusEl} of adj.files){
      try{
        ...
        documentos.push({
          ...
          fileId:res.fileId,
          tipo:esOficioDoc?'oficio_firmado':(esSoporteNotif?'soporte_notificacion':'archivo'),
          mime:file.type||''
        });
        idxFile++;
      }catch(err){
  ...
  const histNotaCanal=isPersonal
    ?('Notificación '+medioNotificacionRespLabel(canal)+' el '+fechaResp+(oficioExt?' · Oficio '+oficioExt:''))
    :(cuerpo||'Respuesta registrada');
  const wfPatch={
    fase:PQRS_WF.CERRADA,
    tipo:tipoResp,
    canal,
    cuerpo:cuerpo||histNotaCanal,
    oficio:oficioExt,
    fecha_respuesta:fechaResp,
    fecha_notificacion:isPersonal?fechaResp:fechaResp,
    documentos,
  ...
  const soporteRes=await _pqrsSubirSoporteRespuesta(e,{fechaResp,cuerpo:wfPatch.cuerpo,documentos,cerradoPor:wfPatch.cerrado_por});
  ...
  registrarPqrsRespuestaCore(e,{fechaResp,oficioExt,medioResp:canal,cuerpo:wfPatch.cuerpo,tipo:tipoResp,canal,notaInterna,esNotaPublica:false,adj:{links:adj.links,files:[]},archivos:documentos});
  if(isPersonal){
    if(!Array.isArray(e._pqrs_historial))e._pqrs_historial=[];
    e._pqrs_historial.push({
      tipo:'notificacion_'+canal,
      fecha:fechaResp,
      nota:histNotaCanal+' — '+pqrsComentarioAutor(),
      oficina:e._pqrs_oficina,
      por:pqrsComentarioAutor()
    });
  }
  persistExpedienteGranular(e);
  if(btn){btn.disabled=false;btn.textContent='✅ Confirmar y cerrar PQRSD';}
...
async function submitPqrsRespuestaPorCorreo(expId){
  ...
  const oficioExt=String((document.getElementById('pqrs-resp-oficio')||{}).value||'').trim();
  const tipoResp=String((document.getElementById('pqrs-resp-tipo')||{}).value||PQRS_WF_TIPO.MENSAJE).trim();
  const toRaw=String((document.getElementById('pqrs-compose-to')||{}).value||'').trim();
  const ccRaw=String((document.getElementById('pqrs-compose-cc')||{}).value||'').trim();
  const bccRaw=String((document.getElementById('pqrs-compose-bcc')||{}).value||'').trim();
  ...
  let atts=(window._pqrsComposeAttachments||[]).slice();
  if(tipoResp===PQRS_WF_TIPO.OFICIO){
    if(!pqrsValidateOficioRespuesta(tipoResp,{requireAdj:true,adjMsg:'Adjunte el PDF del oficio firmado'}))return;
    const adj=collectPqrsRespAdjuntos();
  ...
  if(btn){btn.disabled=false;btn.textContent='📤 Enviar correo y cerrar';}
}
...
function guardarPqrsRespuestaDatos(e,opts,cerrar){
  ...
  if(cerrar){
    ...
    if(opts.emailTo!==undefined)wfPatch.email_to=opts.emailTo;
    if(opts.emailCc!==undefined)wfPatch.email_cc=opts.emailCc;
    ...
    if(tipo===PQRS_WF_TIPO.OFICIO&&opts.notificarPor){
      wfPatch.notificar_por=String(opts.notificarPor).trim();
      wfPatch.notificar_por_propuesto=String(opts.notificarPor).trim();
    }
    setPqrsWorkflow(e,wfPatch);
...
function renderPqrsEntregaCamposHtml(e){
  ...
  h+='<div style="font-size:11px;color:var(--tx2);margin-bottom:8px">Cuando el encargado revise y apruebe, se registrará la respuesta. Verifique el correo de destino si el oficio indica uno distinto al de radicación.</div>';
  ...
  h+='<div class="fld" id="pqrs-entrega-canal-row" style="margin-bottom:8px'+(esInformativa?';display:none':'')+'"><label style="font-size:11px;font-weight:600">Canal de notificación</label>'+
    ...
    mkCan(PQRS_WF_CANAL.CORREO,'Correo','📧')+
    mkCan(PQRS_WF_CANAL.PRESENCIAL,'Presencial / física','🤝')+
    mkCan(PQRS_WF_CANAL.WHATSAPP,'WhatsApp','💬')+
    mkCan(PQRS_WF_CANAL.AVISO,'Por aviso','📌')+
    '</div><input type="hidden" id="pqrs-resp-canal" value="'+escAttr(canalActual||canalDef)+'"></div>';
  ...
  h+='<div class="fld"><label id="pqrs-entrega-cuerpo-label" style="font-size:11px;font-weight:600">'+
    (esInformativa
      ...
      :'Mensaje / plantilla de respuesta <span class="req-star">*</span>')+
    '</label>'+
    '<textarea id="pqrs-entrega-resp-cuerpo" placeholder="'+(esInformativa?'Describa por qué es informativa. El ciudadano verá este texto al consultar…':'Plantilla de respuesta…')+'" style="min-height:180px;padding:8px;border:1px solid var(--bd);border-radius:var(--r);font-size:13px;font-family:\'DM Sans\',sans-serif;width:100%;margin-top:4px;line-height:1.45;box-sizing:border-box">'+escAttr(wf.cuerpo||e._pqrs_respuesta_nota||'')+'</textarea>'+
    ...
    '</div>';
  // Sugerencia de quién notificará (solo oficio firmado)
  const notifWrapStyle=tipoActual===PQRS_WF_TIPO.OFICIO?'':'display:none';
  h+='<div id="pqrs-entrega-notif-wrap" style="margin-top:10px;'+notifWrapStyle+'">'+
    (typeof _pqrsOpcionesNotificadorHtml==='function'
      ?_pqrsOpcionesNotificadorHtml(e,wf,wf.notificar_por||wf.notificar_por_propuesto||responsableActivo||wf.entregado_por,{modo:'entrega',id:'pqrs-entrega-notif-por'})
      :'')+
    '</div>';
...
function pqrsEntregaOnFileChange(inp){
  ...
  box.innerHTML='<div class="fx" style="gap:6px;align-items:center;margin-top:4px;font-size:12px;padding:4px 6px;background:var(--sf2);border-radius:var(--r)">'+
    '📎 <span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+escAttr(f.name)+'</span>'+
...
function onPqrsEntregaOficioBlur(){
  ...
  if(!oficio||oficio.length<4)return;
  const expId=String((window._taskModalCtx||{}).expId||'').trim();
  if(typeof validarNumeroOficioDisponible==='function')validarNumeroOficioDisponible(oficio,expId);
}
function pqrsEntregaRefreshUi(){
  ...
  let canal=String((canalHid&&canalHid.value)||PQRS_WF_CANAL.CORREO);
  const isInfo=tipo===PQRS_WF_TIPO.INFORMATIVA;
  const isOficio=tipo===PQRS_WF_TIPO.OFICIO;
  const isMensaje=tipo===PQRS_WF_TIPO.MENSAJE;
  // Mensaje simple solo por correo
  if(!isInfo&&isMensaje&&!pqrsEsCanalCorreo(canal)){
    if(canalHid)canalHid.value=PQRS_WF_CANAL.CORREO;
  ...
  const oficioHint=document.getElementById('pqrs-entrega-oficio-hint');
  const emailCompose=document.getElementById('pqrs-entrega-email-compose');
  const adjWrap=document.getElementById('pqrs-entrega-adj-wrap');
  const adjLabel=document.getElementById('pqrs-entrega-adj-label');
  ...
  const notifWrap=document.getElementById('pqrs-entrega-notif-wrap');
  if(canalRow)canalRow.style.display=isInfo?'none':'';
  if(oficioRow)oficioRow.style.display=isInfo?'none':'';
  if(notifWrap)notifWrap.style.display=isOficio?'':'none';
  if(oficioReq)oficioReq.style.display=isOficio?'':'none';
  if(oficioHint){
  ...
  // Regenerar selector de notificador según canal (correo → solo VITAL/Encargado)
  if(notifWrap&&isOficio){
    const ctx=window._taskModalCtx||{};
    const eN=typeof getExpById==='function'?getExpById(ctx.expId):null;
    const wfN=eN&&typeof getPqrsWorkflow==='function'?getPqrsWorkflow(eN):{};
    const selPrev=String((document.getElementById('pqrs-entrega-notif-por')||{}).value||wfN.notificar_por||wfN.notificar_por_propuesto||'').trim();
    notifWrap.innerHTML=typeof _pqrsOpcionesNotificadorHtml==='function'
      ?_pqrsOpcionesNotificadorHtml(eN,wfN,selPrev,{modo:'entrega',id:'pqrs-entrega-notif-por',canal:canal})
      :'';
  }
  ...
  if(cuerpoLbl){
    ...
    else if(isOficio)cuerpoLbl.innerHTML='Nota interna <span style="font-weight:400;color:var(--tx3)">(opcional — la plantilla de correo la arma VITAL/NCA al notificar)</span>';
    else if(isCorreo)cuerpoLbl.innerHTML='Mensaje / plantilla de respuesta <span class="req-star">*</span>';
    else cuerpoLbl.innerHTML='Resumen de la respuesta <span class="req-star">*</span>';
  }
  if(cuerpoTxt){
    if(isInfo){
      ...
      // Responsable: solo N° oficio + archivo + anexos; sin plantilla de correo
      if(_pqrsEsPlantillaRespuesta(cuerpoTxt.value))cuerpoTxt.value='';
      cuerpoTxt.placeholder='Opcional: nota breve para el encargado (no es el correo al ciudadano)…';
...
function collectPqrsEntregaDatos(expId){
  ...
  const emailSubject=String((document.getElementById('pqrs-entrega-email-subject')||{}).value||'').trim();
  const notificarPorRaw=tipo===PQRS_WF_TIPO.OFICIO
    ?String((document.getElementById('pqrs-entrega-notif-por')||{}).value||'').trim()
    :'';
  const notificarPor=tipo===PQRS_WF_TIPO.OFICIO
    ?pqrsAplicarReglaNotificadorCanal(e,canal,notificarPorRaw)
    :'';
  if(tipo===PQRS_WF_TIPO.OFICIO&&pqrsEsCanalCorreo(canal)&&!notificarPor){
  ...
  if(!cuerpo){
    ...
    if(tipo===PQRS_WF_TIPO.OFICIO){
      // Oficio: no exige cuerpo; se usa el N° de oficio (plantilla de correo la arma VITAL/NCA al notificar)
    }else{
      notif('Escriba el mensaje o resumen de la respuesta elaborada','err');
      return null;
    }
  }
  const cuerpoFinal=cuerpo||(tipo===PQRS_WF_TIPO.OFICIO?('Oficio '+(oficioExt||'')).trim():'');
  if(!cuerpoFinal&&tipo!==PQRS_WF_TIPO.OFICIO){
    notif('Escriba el mensaje o resumen de la respuesta elaborada','err');
    return null;
  }
  if(tipo===PQRS_WF_TIPO.OFICIO){
  ...
  if(tipo!==PQRS_WF_TIPO.INFORMATIVA&&pqrsEsCanalCorreo(canal)){
    ...
    if(!toList.length){
      notif('Verifique o diligencie el correo de destino (Para)','err');
      const el=document.getElementById('pqrs-entrega-email-to');
      if(el){el.focus();el.classList.add('fld-invalid');}
      return null;
    }
  }
...
function estadoTaskForAsignado(t,nombre){
  ...
  if(eExp&&typeof taskEsAtenderPqrs==='function'&&taskEsAtenderPqrs(t,eExp)
    ...
    &&typeof pqrsFasePostAprobacionProyeccion==='function'&&pqrsFasePostAprobacionProyeccion(eExp)){
    // Notificador designado: su deuda empieza al quedar el oficio firmado (fase notificar)
    if(typeof pqrsEnFaseNotificacion==='function'&&pqrsEnFaseNotificacion(eExp)
      &&typeof pqrsEsNotificadorDesignado==='function'&&pqrsEsNotificadorDesignado(eExp,nombre)){
...
function submitEnviarSoporteVerificacion(expId,taskId){
  ...
  const runSubmit=function(driveArchivos){
    ...
    if(esPqrs&&pq){
      ...
      guardarPqrsRespuestaDatos(e,{fechaResp:pq.fechaResp,oficioExt:pq.oficioExt,cuerpo:pq.cuerpo,tipo:pq.tipo,canal:canalFinal,nota:cmt,adj:pq.adj,archivos:adjDocumentos,emailTo:pq.emailTo,emailCc:pq.emailCc,emailBcc:pq.emailBcc,emailSubject:pq.emailSubject,notificarPor:pq.notificarPor},false);
      persistExpedienteGranular(e,false);
      renderPqrsOficinaInbox();
      renderSecretariaPqrs();
  ...
  if(!sol){
    ...
    if(showDriveUp&&!esPqrsEntrega){
      h+='<div style="margin-bottom:8px"><label style="font-size:11px;font-weight:600;color:var(--tx3)">Subir archivo (Drive institucional — opcional)</label>'+
        ...
        '<div style="font-size:10px;color:var(--tx3);margin-top:2px">Documento principal. También puede pegar un enlace externo abajo.</div></div>'+
...
function openEnviarSoporteModal(expId,taskId,modo){
  ...
  if(esPqrsEntrega){
    setTimeout(function(){
      ...
      const tipo=String((document.getElementById('pqrs-resp-tipo')||{}).value||'');
      // Solo mensaje simple lleva plantilla en entrega; oficio: N° + archivo + anexos (plantilla al notificar)
      if(tipo===PQRS_WF_TIPO.MENSAJE&&typeof pqrsAplicarPlantillaSegunTipo==='function'){
        const canal=String((document.getElementById('pqrs-resp-canal')||{}).value||'');
        if(typeof pqrsEsCanalCorreo==='function'&&pqrsEsCanalCorreo(canal))pqrsAplicarPlantillaSegunTipo(tipo,false);
      }
    },40);
  }
...
function renderNcaDecisionFormHtml(expId,e,wf){
  ...
  return hintCompare+
    ...
    '<input type="text" id="nca-rev-comentario" placeholder="'+(esInfo?'Ej: Aprobado como informativa.':(esOficio?'Ej: Aprobado. Para imprimir o para firma.':'Ej: Aprobado. Proceda a enviar.'))+'" style="margin-top:4px"></div>'+
    (esOficio?_pqrsOpcionesNotificadorHtml(e,wf,wf.notificar_por||wf.notificar_por_propuesto||wf.entregado_por,{modo:'revision',canal:canalRevision}):'')+
...
function renderTaskVerifyBarHtml(expId,taskId,t){
  ...
  if(esPqrs&&e&&!pqrsEstaCerrada(e)){
    ...
    if(pqrsEnRevisionNca(e)){
      ...
      hint='Compare documentos arriba (proyección y anexos). Decida: devolver, para imprimir o atajo para firma (digital). Tipo propuesto: <strong>'+escAttr(tipoLbl)+'</strong>.';
      inlineNca=renderNcaDecisionFormHtml(expId,e,wf);
    }else if(pqrsEnParaFirma(e)){
      titulo='🖨 Por imprimir — PQRSD';
      hint='Oficio aprobado. VITAL o NCA lo visualizan en «Por imprimir» y pueden pasar a firma del Director.';
      if(typeof pqrsPuedeMarcarParaFirma==='function'&&pqrsPuedeMarcarParaFirma(e))
        btns='<button type="button" class="btn bsm bp" style="background:#1a7a4a;border-color:#1a7a4a" onclick="openPqrsParaFirmaModal(\''+jsStr(expId)+'\')">🖨 Pasar a firma</button>';
      else btns='<span style="font-size:11px;color:var(--tx2)">Pendiente de VITAL / NCA</span>';
    }else if(fase===PQRS_WF.POR_FIRMAR){
      titulo='🖊 Por firmar — Director';
      const quien=wf.notificar_por?' Cuando se asigne, notificará: <strong>'+escAttr(wf.notificar_por)+'</strong>.':'';
      const imp=wf.impreso&&wf.impreso.en?' Oficio marcado <strong>impreso</strong>.':'';
      const firmFis=wf.firma_fisica&&wf.firma_fisica.en;
      hint=(firmFis
        ?('✓ Firmado físico registrado. Pendiente que VITAL / NCA / encargado pase a <strong>Por notificar</strong>.')
        :('Pendiente de firma del Director (DS DEGUV).'+imp+quien+' Si ya firmó en físico, VITAL / encargado pueden marcar <strong>ya firmado</strong> sin subir PDF y priorizar el oficio notificado al notificar.'));
      if(firmFis&&typeof pqrsPuedeAsignarPorNotificar==='function'&&pqrsPuedeAsignarPorNotificar(e))
        btns='<button type="button" class="btn bsm act-ico bp" onclick="pqrsPasarFirmadoAPorNotificar(\''+jsStr(expId)+'\')" title="Pasar a por notificar">📬</button>';
      else if(firmFis&&typeof esDirectorDsDeguv==='function'&&esDirectorDsDeguv())
        btns='<span class="btn bsm act-ico" style="background:#185fa522;color:var(--bl);cursor:default" title="Firmado — pendiente de notificación">📬</span>';
      else if(typeof pqrsPuedeFirmarDirector==='function'&&pqrsPuedeFirmarDirector(e))
        btns='<button type="button" class="btn bsm act-ico bp" onclick="openPqrsDirectorFirmarModal(\''+jsStr(expId)+'\')" title="Firmar">🖊</button>';
      else if(typeof pqrsPuedeMarcarFirmadoSinCargar==='function'&&pqrsPuedeMarcarFirmadoSinCargar(e))
        btns='<button type="button" class="btn bsm act-ico" style="background:#15803d;color:#fff" onclick="openPqrsDirectorFirmarModal(\''+jsStr(expId)+'\')" title="Ya firmado (sin PDF) / cargar / notificar">✓</button>';
      else btns='<span style="font-size:11px;color:var(--tx2)">'+(firmFis?'Firmado · pendiente asignar notificación':'Visible en menú PQRSD de DS DEGUV · luego «Por notificar»')+'</span>';
      ...
      if(typeof esDirectorDsDeguv==='function'&&esDirectorDsDeguv())
        btns='<span class="btn bsm act-ico" style="background:#185fa522;color:var(--bl);cursor:default" title="Firmado — en proceso de notificación">📬</span>';
      else if(typeof pqrsPuedeNotificarOficio==='function'&&pqrsPuedeNotificarOficio(e))
      ...
      titulo='↩ Devuelta al responsable';
      hint='La respuesta fue devuelta. El responsable debe corregir y volver a entregar.';
    }else{
      titulo='📋 PQRSD en gestión';
      hint='Espere la entrega del responsable o gestione desde el menú PQRSD.';
    }
  ...
  const autoResp=!!(t.autoAsignadaPorResponsable||t.origen==='responsable');
  ...
  if(pendVer&&!esLibre){
    ...
    h+='<div style="margin-bottom:10px;padding:8px;background:var(--sf);border:1px solid var(--bd);border-radius:var(--r)">'+
      ...
      '<div style="font-size:11px;color:var(--tx3);margin-bottom:6px">Indique días o fecha calendario. No cierra la actividad; formaliza el vencimiento. Co-ejecutores y traslado: use el panel de responsables arriba.</div>'+
      '<div class="fx" style="gap:8px;flex-wrap:wrap;align-items:center">'+
        '<label style="font-size:12px">Días <input type="number" id="task-rev-plazo" min="1" step="1" value="'+escAttr(plazoVal)+'" style="width:72px;padding:6px;border:1px solid var(--bd);border-radius:var(--r);margin-left:4px" oninput="syncPlazoRevisionFromDias()"></label>'+
        '<label style="font-size:12px">Vence <input type="date" id="task-rev-vence" value="'+escAttr(t.vence||'')+'" style="padding:6px;border:1px solid var(--bd);border-radius:var(--r);margin-left:4px" onchange="syncPlazoRevisionFromFecha()"></label>'+
        '<button type="button" class="btn bsm" onclick="aplicarPlazoRevisionTask(\''+jsStr(expId)+'\',\''+jsStr(taskId)+'\')">Aplicar plazo</button>'+
      '</div></div>';
  }
  // Selector notificador (libres); con expediente va en renderTramiteFirmaVerifyExtrasHtml
  ...
  if(!enFirmaTram&&pendVer){
    ...
    const tituloDec=esLibre?'Decisión (sin expediente)':'Decisión (trámite)';
    const hintDec=esLibre
      ?'Oficios simples (invitación, etc.) no requieren radicar ni asociar expediente. Elija el flujo de firma o cierre directo.'
      :'Elija el flujo: imprimir → firma del Director, atajo a firma, cargar documento ya firmado (Por notificar), o cerrar sin firma.';
    h+='<div style="margin-bottom:10px;padding:10px;background:#0d5c2e12;border:1px solid #0d5c2e;border-radius:var(--r)">'+
      '<div style="font-size:12px;font-weight:600;color:#0d5c2e;margin-bottom:4px">'+tituloDec+'</div>'+
      '<div style="font-size:11px;color:var(--tx2);margin-bottom:8px">'+hintDec+'</div>'+
      '<div style="font-size:12px;font-weight:600;margin-bottom:6px">Decisión:</div>'+
      '<div class="fx" style="gap:8px;flex-wrap:wrap;align-items:center">';
    if(puedeImprimir)
...
function syncPlazoRevisionFromDias(){
  const p=document.getElementById('task-rev-plazo');
  const v=document.getElementById('task-rev-vence');
  if(!p||!v)return;
  if(p.value===''||p.value===null)return;
  if(typeof calcVence==='function')v.value=calcVence(p.value);
}
function syncPlazoRevisionFromFecha(){
  const p=document.getElementById('task-rev-plazo');
  const v=document.getElementById('task-rev-vence');
  if(!p||!v||!v.value)return;
  if(typeof diffDias==='function')p.value=diffDias(v.value);
}
function aplicarPlazoRevisionTask(expId,taskId){
  const pEl=document.getElementById('task-rev-plazo');
  const vEl=document.getElementById('task-rev-vence');
  let plazo=pEl?String(pEl.value||'').trim():'';
  let vence=vEl?String(vEl.value||'').trim():'';
  if(plazo&&!vence&&typeof calcVence==='function')vence=calcVence(plazo);
  if(vence&&!plazo&&typeof diffDias==='function')plazo=String(diffDias(vence));
  if(!vence){notif('Indique días o fecha de vencimiento','err');return false;}
  const ok=mutateTask(expId,taskId,function(t){
    ...
    t.historial.push({
      ...
      nota:'Plazo fijado en revisión: '+(plazo?plazo+' día(s) · ':'')+'vence '+vence
    });
    if(typeof syncTaskAggregateState==='function')syncTaskAggregateState(t);
  });
  if(ok){
    notif('Plazo aplicado · vence '+fmtF(vence),'ok');
    openTaskCommentsModal(expId,taskId);
  }
  return ok;
}
...
function confirmarCierreTask(expId,taskId){
  ...
  if(e&&t&&taskEsAtenderPqrs(t,e)&&!pqrsEstaCerrada(e)){
    const fase=pqrsWorkflowFase(e);
    if(pqrsEnRevisionNca(e)&&typeof openNcaRevisionModal==='function'){openNcaRevisionModal(expId);return;}
    if(pqrsEnParaFirma(e)&&typeof openPqrsParaFirmaModal==='function'){openPqrsParaFirmaModal(expId);return;}
    if(fase===PQRS_WF.POR_FIRMAR&&typeof openPqrsDirectorFirmarModal==='function'){openPqrsDirectorFirmarModal(expId);return;}
    if((fase===PQRS_WF.PENDIENTE_NOTIF||fase===PQRS_WF.LISTA_ENVIO)&&typeof openPqrsNotificarOficioModal==='function'){openPqrsNotificarOficioModal(expId);return;}
...
function estadoTaskLabel(t){
  ...
  if(esModoResponsable()&&responsableActivo&&typeof taskUsuarioEsAsignado==='function'&&taskUsuarioEsAsignado(t,responsableActivo)){
    ...
    if(estP==='Atendida'){
      const eExp=typeof getExpById==='function'?getExpById(t&&(t.exp||t.codigo)):null;
      if(eExp&&typeof pqrsFasePostAprobacionProyeccion==='function'&&pqrsFasePostAprobacionProyeccion(eExp)
        &&!(typeof pqrsEstaCerrada==='function'&&pqrsEstaCerrada(eExp)))
        return'Atendida';
    }
  }
  ...
  if(typeof taskFirmaEstadoUi==='function'){
    ...
    if(uiT&&uiT.lbl)return uiT.sub?(uiT.lbl+' · '+uiT.sub):uiT.lbl;
  }
...
function taskEstadoStyle(est,tOpt){
  if(tOpt&&esModoResponsable()&&responsableActivo&&typeof taskUsuarioEsAsignado==='function'&&taskUsuarioEsAsignado(tOpt,responsableActivo)
    &&estadoTaskForAsignado(tOpt,responsableActivo)==='Atendida'){
    return{bg:'var(--gnl)',fg:'var(--gn)'};
  }
  if(tOpt){
    ...
    if(typeof taskFirmaEstadoUi==='function'){
      const uiT=taskFirmaEstadoUi(tOpt);
      if(uiT&&uiT.lbl)return{bg:uiT.bg,fg:uiT.fg};
    }
  }
  ...
  const doVerify=function(){
    ...
    t.historial.push({tipo:'verificacion',fecha:fechaC,ts:Date.now(),por:taskComentarioAutor(),nota:repPend?('Aprobada entrega de '+repPend):'Actividad aprobada y cerrada',reportadoPor:repPend||''});
      ...
      if(expRec&&tDone&&!(typeof esPqrsSecretaria==='function'&&esPqrsSecretaria(expRec))){
        mutateTask(expId,taskId,function(tk){tk.publicado=true;});
        if(typeof notificarCiudadanoTrasVerificarTramite==='function'){
...
function submitTaskComment(expId,taskId){
  ...
  if(addTaskComentario(expId,taskId,inp.value)){
    ...
    renderBandejaDepto();
    if(document.getElementById('pg-act')&&document.getElementById('pg-act').classList.contains('on'))renderActividades();
    if(modePrev==='pqrsDirectorFirmar'&&typeof openPqrsDirectorFirmarModal==='function'){
...
/** Si el Director devolvió y la tarea quedó Atendida/Aprobada, reabrir a Por verificar (una sola vez). */
function pqrsHealTaskTrasDevolucionDirector(e,t){
  if(!e||!t||typeof pqrsEnRevisionNca!=='function'||!pqrsEnRevisionNca(e))return false;
  const wf=typeof getPqrsWorkflow==='function'?getPqrsWorkflow(e):{};
  if(!(wf.devolucion_director&&wf.devolucion_director.motivo))return false;
  normalizeTask(t);
  migrateLegacyAsignados(t);
  const est=estadoTask(t);
  const stillOk=est==='Por verificar'&&!t.fechaAtendida&&!t.ultimaRevisionDepto
    &&(t.asignados||[]).every(function(a){return!a||(a.estado==='por_verificar'&&!a.fechaAtendida);});
  if(stillOk)return false;
  t._pqrs_proyeccion_atendida=false;
  t.fechaAtendida='';
  t.verificadoPor='';
  t.ultimaRevisionDepto=null;
  const fr=t.fechaReportada||hoy();
  t.fechaReportada=fr;
  (t.asignados||[]).forEach(function(a){
    if(!a)return;
    a.fechaAtendida='';
    a.fechaReportada=a.fechaReportada||fr;
    a.estado='por_verificar';
  });
  if(typeof syncTaskAggregateState==='function')syncTaskAggregateState(t);
  t.estado='Por verificar';
...
function renderActividadesRowHtml(t){
  const expActPre=typeof getExpById==='function'?getExpById(t.exp):null;
  if(expActPre&&typeof taskEsAtenderPqrs==='function'&&taskEsAtenderPqrs(t,expActPre)){
    const realT=(expActPre.tasks||[]).find(function(x){return x&&String(x.id)===String(t.id);});
    if(realT&&pqrsHealTaskTrasDevolucionDirector(expActPre,realT)){
      ...
      if(!window._pqrsHealDevDirPersist)window._pqrsHealDevDirPersist=new Set();
      const hk=String(expActPre._exp||'')+'|'+String(realT.id||'');
  ...
  const canRevisarDept=esVistaActividadesDepto()&&!esModoResponsable()&&(
    taskPendienteVerificacion(t)
  ...
  // Flujo firma de trámite (no PQRSD)
  if(typeof taskEnFlujoFirmaTramite==='function'&&taskEnFlujoFirmaTramite(t)){
    const eidT=jsStr(t.exp),tidT=jsStr(t.id);
  ...
  if(expAct&&typeof esPqrsSecretaria==='function'&&esPqrsSecretaria(expAct)&&typeof pqrsWorkflowFase==='function'){
    ...
    if(typeof pqrsEnParaFirma==='function'&&pqrsEnParaFirma(expAct)&&typeof pqrsPuedeMarcarParaFirma==='function'&&pqrsPuedeMarcarParaFirma(expAct)){
      ...
      acts+='<button type="button" class="btn bsm act-ico" style="background:#1a7a4a;color:#fff" onclick="event.stopPropagation();openPqrsParaFirmaModal(\''+eid+'\')" title="Imprimir y pasar a firma del Director">🖊</button>';
    }
    ...
    if(faseWf===PQRS_WF.PENDIENTE_NOTIF||faseWf===PQRS_WF.LISTA_ENVIO){
      acts+=_pqrsActDocsIconBtnsHtml(wfRow,{expId:expAct._exp||t.exp});
      if(typeof esDirectorDsDeguv==='function'&&esDirectorDsDeguv()){
        acts+='<span class="btn bsm act-ico" style="background:#185fa522;color:var(--bl);cursor:default;pointer-events:none" title="Firmado — en proceso de notificación">📬</span>';
      }else if(typeof pqrsPuedeNotificarOficio==='function'&&pqrsPuedeNotificarOficio(expAct))
        acts+='<button type="button" class="btn bsm act-ico bp" onclick="event.stopPropagation();openPqrsNotificarOficioModal(\''+eid+'\')" title="Notificar">📬</button>';
      else{
...
function updateActEstFilterForEnc(forEnc){
  ...
  const puedeImprimir=typeof pqrsPuedeFlujoPorImprimir==='function'&&pqrsPuedeFlujoPorImprimir();
  const puedeFirmarBan=typeof pqrsPuedeFlujoPorFirmarBandeja==='function'&&pqrsPuedeFlujoPorFirmarBandeja();
  ...
  if(puedeFirmarBan&&order.indexOf('porfirmar')<0){
...
function esActividadPorEjecutar(t){
  ...
  const isVital=typeof esCargoVital==='function'&&esCargoVital();
  const deptView=typeof esVistaActividadesDepto==='function'&&esVistaActividadesDepto();
  // Encargado (filtro = sí mismo): al asignar a otro responsable, sale de «Por ejecutar»
  if(deptView){
    const enc=typeof getEncargadoDepto==='function'?String(getEncargadoDepto(deptoActivo)||'').trim():'';
    const filt=typeof getActDeptRespFilter==='function'?String(getActDeptRespFilter()||'').trim():'';
    if(enc&&filt&&typeof agendaNorm==='function'&&agendaNorm(filt)===agendaNorm(enc)){
      const rs=typeof getTaskResponsables==='function'?getTaskResponsables(t):[];
      const otros=(rs||[]).filter(function(n){return n&&agendaNorm(n)!==agendaNorm(enc);});
      if(otros.length){
        if(typeof taskUsuarioEsAsignado!=='function'||!taskUsuarioEsAsignado(t,enc))return false;
        const stEnc=estadoTaskForAsignado(t,enc);
...
async function ncaAprobarOficioFirmado(expId){
  ...
  notifPor=pqrsAplicarReglaNotificadorCanal(e,wf.canal||PQRS_WF_CANAL.CORREO,notifPor);
  ...
  renderSecretariaPqrs();
  if(typeof renderActividades==='function')renderActividades();
  notif('🖨 Oficio aprobado — queda en «Por imprimir»'+(notifPor?' · Notificará: '+notifPor:''),'ok');
}

/** Atajo NCA: aprobar oficio y enviar directo a «Por firmar» (firma digital, sin impresión). */
async function ncaAprobarOficioParaFirmaDirecto(expId){
  if(typeof guardMantenimientoSoloConsulta==='function'&&guardMantenimientoSoloConsulta())return;
  const e=exps.find(x=>String(x._exp||'').trim()===String(expId||'').trim());
  if(!e){notif('PQRSD no encontrada','err');return;}
  if(!(typeof pqrsPuedeAtajoParaFirma==='function'&&pqrsPuedeAtajoParaFirma())){
    notif('No puede usar el atajo «Para firma»','err');return;
  }
  const d=_ncaRevisionDatos();
  const wf=getPqrsWorkflow(e);
  let notifPor=_pqrsLeerNotifPorSel()||String(wf.notificar_por||wf.notificar_por_propuesto||'').trim();
  notifPor=pqrsAplicarReglaNotificadorCanal(e,wf.canal||PQRS_WF_CANAL.CORREO,notifPor);
  await _pqrsLimpiarVersionesCorreccionWf(e,wf);
  await _pqrsRenombrarDocsDriveWf(wf,'por_firmar',{onlyEstados:['revision','','por_firma','aprobado']});
  const por=responsableActivo||'NCA';
  const patchWf={
    fase:PQRS_WF.POR_FIRMAR,
    cuerpo:d.cuerpo||wf.cuerpo,
    oficio:d.oficio||wf.oficio,
    fecha_respuesta:d.fecha||wf.fecha_respuesta,
    documentos:wf.documentos||[],
    revision_nca:{aprobado:true,tipo:'oficio',comentario:d.comentario,por:por,en:new Date().toISOString(),atajo_firma:true},
    listo_firma:{por:por,en:new Date().toISOString(),atajo_digital:true},
    impreso:null,
    firma_fisica:null,
    firma_director:null
  ...
  setPqrsWorkflow(e,patchWf);
  if(!Array.isArray(e._pqrs_historial))e._pqrs_historial=[];
  e._pqrs_historial.push({tipo:'revision_nca_aprobado_oficio_firma',fecha:hoy(),nota:'NCA aprobó oficio — atajo «Para firma» (sin imprimir)'+(notifPor?' · Notificará: '+notifPor:'')+(d.comentario?' — '+d.comentario:''),oficina:'guaviare',por:por});
  pqrsSincronizarParticipacionPostAprobacion(e);
  if(typeof clearAltaResponsableAlAprobarDocumento==='function')clearAltaResponsableAlAprobarDocumento(expId,{force:true});
  persistExpedienteGranular(e);
  closeTaskModal();
  ...
  renderSecretariaPqrs();
  if(typeof renderActividades==='function')renderActividades();
  notif('🖊 Oficio aprobado — queda en «Por firmar» (atajo digital)'+(notifPor?' · Notificará: '+notifPor:''),'ok');
}
...
// ===========================================================================
// Oficio firmado: Para firma → Por firmar (Director) → Por notificar
// ===========================================================================
function openVitalBandejaModal(expId){
  // Compat: redirige según fase actual
  const e=exps.find(x=>String(x._exp||'').trim()===String(expId||'').trim());
  if(!e){notif('PQRSD no encontrada','err');return;}
  const f=pqrsWorkflowFase(e);
  if(pqrsEnParaFirma(e))return openPqrsParaFirmaModal(expId);
  if(f===PQRS_WF.POR_FIRMAR)return openPqrsDirectorFirmarModal(expId);
  if(f===PQRS_WF.PENDIENTE_NOTIF||f===PQRS_WF.LISTA_ENVIO)return openPqrsNotificarOficioModal(expId);
  notif('Esta PQRSD no está en una fase de firma/notificación','err');
}

/**
 * Selector de quién notificará al ciudadano.
 * opts.modo: 'entrega'|'revision'|'firma'|'director' — ajusta texto de ayuda
 * opts.id: id del <select> (default pqrs-notif-por-sel)
 */
function _pqrsOpcionesNotificadorHtml(e,wf,selected,opts){
  opts=opts||{};
  const modo=String(opts.modo||'firma');
  const selId=String(opts.id||'pqrs-notif-por-sel');
  ...
  [...names].sort((a,b)=>a.localeCompare(b,'es')).forEach(n=>{
    ...
    optsHtml+='<option value="'+escAttr(n)+'"'+(cur===n?' selected':'')+'>'+escAttr(n)+(tags.length?' · '+tags.join(' · '):'')+'</option>';
  });
...
function openPqrsParaFirmaModal(expId){
  ...
  body.innerHTML=
    ...
    '<div style="font-size:11px;color:var(--tx2)">Oficio: <strong>'+escAttr(wf.oficio||'—')+'</strong> · Canal previsto: '+escAttr(wf.canal||'—')+'</div>'+
...
async function pqrsPasarAFirmaDirector(expId){
  ...
  const wf=getPqrsWorkflow(e);
  let notifPor=String((document.getElementById('pqrs-notif-por-sel')||{}).value||'').trim();
...
function _pqrsDocDriveUrls(doc){
  const raw=String((doc&&(doc.driveLink||doc.previewLink||doc.url))||'').trim();
  if(!raw)return{preview:'',view:'',download:''};
  const p=typeof parseDrivePreviewUrl==='function'?parseDrivePreviewUrl(raw):null;
  const view=p&&p.url?p.url:String(raw).replace(/\/preview(\?.*)?$/,'/view');
  const preview=p&&p.preview?p.preview:String(raw).replace(/\/view(\?.*)?$/,'/preview');
  const id=(p&&p.id)||'';
  const download=id?('https://drive.google.com/uc?export=download&id='+encodeURIComponent(id)):view;
  return{preview,view,download};
}
...
function _pqrsDirectorDocsFirmaCtx(e){
  const wf=getPqrsWorkflow(e);
  ...
  return{wf,docs,docFirma,anexosFirma,urls};
}
function _pqrsDirectorPreviewBlockHtml(docFirma,urls,hint){
  if(urls&&urls.preview){
    return '<div style="margin-bottom:12px">'+
      ...
      '<div class="pqrs-firma-preview"><iframe title="Vista del oficio" src="'+escAttr(urls.preview)+'"></iframe></div>'+
      ...
      (hint?'<span style="font-size:11px;color:var(--tx3)">'+hint+'</span>':'')+
      '</div></div>';
  }
  return '<div style="padding:10px;background:var(--rdl);border-radius:var(--r);margin-bottom:12px;font-size:12px">No hay documento en Drive para previsualizar. Solicite a VITAL/encargado que pase de nuevo a firma con el archivo.</div>';
}
function _pqrsDirectorAnexosBlockHtml(anexosFirma){
  if(!anexosFirma||!anexosFirma.length)return'';
  return '<div style="margin:0 0 12px;padding:10px;background:var(--sf2);border:1px solid var(--bd);border-radius:var(--r)">'+
...
function _pqrsDirectorVerSoloLecturaHtml(e,ctx){
  const docSol=typeof htmlPqrsDocumentoBtns==='function'?htmlPqrsDocumentoBtns(e):'';
  const traz=typeof renderPqrsTrazabilidadHtml==='function'?renderPqrsTrazabilidadHtml(e):'';
  let docsExp='';
  if(typeof collectEnlacesExpediente==='function'){
    ...
    if(links.length){
      docsExp='<div class="pqrs-det-sec"><div class="pqrs-det-k">Documentos del expediente</div>'+
        links.map(function(l){
          ...
          const lbl=l.label||l.nombre||l.tipo||'Documento';
          return '<div style="font-size:12px;margin-bottom:3px">📄 <a href="'+escAttr(u)+'" target="_blank" rel="noopener">'+escAttr(lbl)+'</a></div>';
        }).join('')+'</div>';
    }
  }
  const asocReadonly=typeof renderPqrsAsociadosPanelHtml==='function'?renderPqrsAsociadosPanelHtml(e):'';
  return _pqrsDirectorPreviewBlockHtml(ctx.docFirma,ctx.urls,'Solo lectura: revise el documento que firmará.')+
    _pqrsDirectorAnexosBlockHtml(ctx.anexosFirma)+
    _pqrsDirectorInfoFirmaReadonlyHtml(e,ctx.wf)+
    (docSol?('<div class="pqrs-det-sec"><div class="pqrs-det-k">Solicitud inicial PQRSD</div><div class="fx" style="gap:6px;flex-wrap:wrap">'+docSol+'</div></div>'):'')+
    '<div class="pqrs-det-sec"><div class="pqrs-det-k">Asunto</div><div class="pqrs-det-v">'+escAttr(e.f_f1||e._pqrs_detalle||'—')+'</div></div>'+
    asocReadonly+
    docsExp+
    traz+
    '<div style="font-size:11px;color:var(--tx3);margin-top:8px">Vista del Director: no puede asignar responsable, marcar informativa ni asociar PQRSD/expedientes desde aquí.</div>';
}
/**
 * Modal Director (DS DEGUV) — modos: ver | cargar | ya_firmado | devolver
 * Sin modo (o modo completo): UI VITAL/encargado (ya firmado + asignar notif).
 */
function openPqrsDirectorFirmarModal(expId,mode){
  if(typeof guardMantenimientoSoloConsulta==='function'&&guardMantenimientoSoloConsulta())return;
  const e=exps.find(x=>String(x._exp||'').trim()===String(expId||'').trim());
  if(!e){notif('PQRSD no encontrada','err');return;}
  ...
  const chatHtml=tAct&&typeof renderTaskChatPanelHtml==='function'
    ?renderTaskChatPanelHtml(expId,tAct.id,tAct)
    :'<div style="font-size:12px;color:var(--tx3)">Sin actividad asociada para chat.</div>';

  const previewBlock=_pqrsDirectorPreviewBlockHtml(ctx.docFirma,ctx.urls,esAtajo?'Documento de referencia. Puede marcar firmado sin subir PDF.':'Lea el documento aquí. Luego cargue el PDF ya firmado.');
...
/** Acciones dedicadas del Director en «Por firmar». */
function openPqrsDirectorAccionModal(expId,mode){
  const e=exps.find(x=>String(x._exp||'').trim()===String(expId||'').trim());
  if(!e){notif('PQRSD no encontrada','err');return;}
  if(!(typeof esDirectorDsDeguv==='function'&&esDirectorDsDeguv())&&!esAdministrador()){
  ...
  const wf=ctx.wf;
  const usaDrive=typeof DRIVE_INST_DEPTOS!=='undefined'&&DRIVE_INST_DEPTOS.has('guaviare');
  let titulo='Por firmar — '+expId;
  let html='';
  if(mode==='ver'){
    titulo='👁 Ver documento — '+expId;
    html='<div style="font-size:13px;font-weight:600;margin-bottom:.35rem">Documento a firmar</div>'+
      '<div style="font-size:11px;color:var(--tx2);margin-bottom:10px">Oficio: <strong>'+escAttr(wf.oficio||'—')+'</strong> · '+(e._tipo_solicitud||'PQRSD')+'</div>'+
      _pqrsDirectorVerSoloLecturaHtml(e,ctx)+
    ...
    html='<div style="font-size:13px;font-weight:600;margin-bottom:.35rem">Cargar PDF ya firmado</div>'+
      ...
      (usaDrive
        ?'<div style="margin-bottom:12px;padding:10px;border:1px solid var(--bd);border-radius:var(--r);background:#0d5c2e08">'+
          '<button type="button" class="btn bsm bp" onclick="pqrsDirectorAddSignedPdf()">📎 Seleccionar PDF firmado</button>'+
          '<input type="file" id="director-pdf-file" accept=".pdf,application/pdf" style="display:none" onchange="pqrsDirectorOnSignedPdf(this)">'+
          '<div id="director-pdf-list" class="pqrs-compose-att-list" style="margin-top:6px"></div></div>'
    ...
    titulo='↩ Devolver documento — '+expId;
    html='<div style="font-size:13px;font-weight:600;margin-bottom:.35rem">Devolver por error / corrección</div>'+
      '<div style="font-size:11px;color:var(--tx2);margin-bottom:10px">El oficio volverá a «Por revisar» para que corrijan y lo envíen de nuevo a firma.</div>'+
      _pqrsDirectorPreviewBlockHtml(ctx.docFirma,ctx.urls,'')+
      '<div style="margin-bottom:12px;padding:10px;border:1px dashed var(--or);border-radius:var(--r);background:#fff7ed">'+
      '<textarea id="director-devolver-motivo" placeholder="Indique el error o qué debe corregirse…" style="width:100%;min-height:80px;padding:8px;border:1px solid var(--bd);border-radius:var(--r);font-size:12px;box-sizing:border-box;font-family:\'DM Sans\',sans-serif"></textarea>'+
      '</div>'+
      '<div class="pqrs-firma-actions">'+
      '<button type="button" class="btn bsm bd2" onclick="pqrsDirectorDevolverParaCorregir(\''+escAttr(expId)+'\')">↩ Devolver documento</button>'+
      '<button type="button" class="btn bsm" onclick="closeTaskModal()">Cancelar</button></div>';
  }
  ...
  window._directorSignedFile=null;
  ov.classList.add('on');
  window._taskModalCtx={mode:'pqrsDirectorAccion',accion:mode,expId};
}
...
async function pqrsDirectorDevolverParaCorregir(expId){
  ...
  if(!e){notif('PQRSD no encontrada','err');return;}
  if(!(typeof esDirectorDsDeguv==='function'&&esDirectorDsDeguv())&&!esAdministrador()){
    notif('Solo el Director puede devolver desde esta vista','err');return;
  }
  if(pqrsWorkflowFase(e)!==PQRS_WF.POR_FIRMAR){notif('La PQRSD ya no está en «Por firmar»','err');return;}
  const motivo=String((document.getElementById('director-devolver-motivo')||{}).value||'').trim();
  ...
  if(t){
    normalizeTask(t);
    migrateLegacyAsignados(t);
    // Quitar «Atendida / Aprobada»: vuelve a revisión del departamento (como primera entrega)
    t._pqrs_proyeccion_atendida=false;
    t.fechaAtendida='';
    t.verificadoPor='';
    t.ultimaRevisionDepto=null;
  ...
  if(typeof renderSecretariaPqrs==='function')renderSecretariaPqrs();
  if(typeof renderActividades==='function')renderActividades();
  notif('↩ Oficio devuelto — queda en «Por revisar» (sin Aprobada/Atendida). Use Revisar para decidir.','ok');
}
...
function pqrsDirectorAddSignedPdf(){
  (typeof sstSolicitarGmailParaAdjuntar==='function'?sstSolicitarGmailParaAdjuntar():Promise.resolve(true)).then(function(ok){
    if(!ok)return;
    const inp=document.getElementById('director-pdf-file');
    if(inp)inp.click();
  });
}
function pqrsDirectorOnSignedPdf(inp){
  const f=inp&&inp.files&&inp.files[0];
  window._directorSignedFile=f||null;
  const box=document.getElementById('director-pdf-list');
  if(!box)return;
  if(!f){box.innerHTML='';return;}
...
/** Director: marca firmado en papel; permanece en «Por firmar» hasta que VITAL/NCA/encargado asigne notificación. */
async function pqrsDirectorMarcarFirmadoFisico(expId,opts){
  opts=opts||{};
  if(typeof guardMantenimientoSoloConsulta==='function'&&guardMantenimientoSoloConsulta())return;
  const e=exps.find(x=>String(x._exp||'').trim()===String(expId||'').trim());
  if(!e){notif('PQRSD no encontrada','err');return;}
  if(!(typeof esDirectorDsDeguv==='function'&&esDirectorDsDeguv())&&!esAdministrador()){
    notif('Solo el Director puede marcar firmado físico desde esta acción','err');return;
  }
  if(pqrsWorkflowFase(e)!==PQRS_WF.POR_FIRMAR){notif('La PQRSD no está en «Por firmar»','err');return;}
...
function pqrsRegistrarFirmadoFisicoSinCargar(e,opts){
  opts=opts||{};
  if(!e)return false;
  const wf=getPqrsWorkflow(e);
  if(wf.firma_fisica&&wf.firma_fisica.en)return true;
  const por=responsableActivo||rolSesion||'VITAL/encargado';
  setPqrsWorkflow(e,{
    fase:PQRS_WF.POR_FIRMAR,
    firma_fisica:{por:por,en:new Date().toISOString(),modo:'fisico',sin_pdf:true,registrado_por:'vital_encargado'}
  });
  if(!Array.isArray(e._pqrs_historial))e._pqrs_historial=[];
  e._pqrs_historial.push({
    tipo:'firma_fisica_vital_encargado',
    fecha:hoy(),
    nota:(opts.nota||'VITAL / encargado registró oficio ya firmado en físico (sin cargar PDF)')+(opts.extra||''),
...
async function pqrsMarcarFirmadoSinCargarYAsignar(expId){
  if(typeof guardMantenimientoSoloConsulta==='function'&&guardMantenimientoSoloConsulta())return false;
  const e=exps.find(x=>String(x._exp||'').trim()===String(expId||'').trim());
  if(!e){notif('PQRSD no encontrada','err');return false;}
...
/** VITAL / encargado: ya firmado sin PDF y abrir notificación de inmediato. */
async function pqrsMarcarFirmadoSinCargarYNotificar(expId){
  if(typeof guardMantenimientoSoloConsulta==='function'&&guardMantenimientoSoloConsulta())return;
  const e=exps.find(x=>String(x._exp||'').trim()===String(expId||'').trim());
  if(!e){notif('PQRSD no encontrada','err');return;}
  if(!pqrsPuedeMarcarFirmadoSinCargar(e)&&!pqrsPuedeAsignarPorNotificar(e)){
  ...
  if(sel&&yo){
    const quien=String(sel.value||'').trim();
    if(!quien||agendaNorm(quien)!==agendaNorm(yo)){
      // Preferir notificarse a sí mismos en este atajo
      const opts=[].slice.call(sel.options||[]);
      const match=opts.find(function(o){return agendaNorm(o.value)===agendaNorm(yo);});
      if(match)sel.value=match.value;
      else if(yo){
        const o=document.createElement('option');
        o.value=yo;o.textContent=yo;o.selected=true;
        sel.appendChild(o);sel.value=yo;
      }
    }
  }
  pqrsRegistrarFirmadoFisicoSinCargar(e,{extra:' · notificación inmediata'});
  const ok=await pqrsPasarFirmadoAPorNotificar(expId,{skipClose:true});
  if(!ok)return;
  await openPqrsNotificarOficioModal(expId);
}

/** VITAL / NCA / encargado: mueve un oficio con firma física a «Por notificar». */
async function pqrsPasarFirmadoAPorNotificar(expId,opts){
  opts=opts||{};
  if(typeof guardMantenimientoSoloConsulta==='function'&&guardMantenimientoSoloConsulta())return false;
  const e=exps.find(x=>String(x._exp||'').trim()===String(expId||'').trim());
  if(!e){notif('PQRSD no encontrada','err');return false;}
  if(opts.ensureFirmaFisica&&pqrsPuedeMarcarFirmadoSinCargar(e)){
    pqrsRegistrarFirmadoFisicoSinCargar(e);
  }
  if(!pqrsPuedeAsignarPorNotificar(e)){notif('No puede asignar esta PQRSD a «Por notificar»','err');return false;}
  const wf=getPqrsWorkflow(e);
  const firm=wf.firma_fisica||{};
  ...
  const patch={
    ...
    documentos:wf.documentos||[],
    firma_director:{por:firm.por||'DS DEGUV',en:firm.en||new Date().toISOString(),modo:'fisico',sin_pdf:!!firm.sin_pdf},
    firma_fisica:null,
  ...
  pqrsSincronizarParticipacionPostAprobacion(e);
  if(!Array.isArray(e._pqrs_historial))e._pqrs_historial=[];
  e._pqrs_historial.push({tipo:'asignado_por_notificar',fecha:hoy(),nota:'Asignado a «Por notificar» tras firma física'+(firm.sin_pdf?' (sin PDF firmado)':'')+' (vence '+fmtF(notifVence)+')'+(notifPor?' · Notificará: '+notifPor:''),oficina:e._pqrs_oficina||'guaviare',por:por});
  persistExpedienteGranular(e);
  if(!opts.skipClose){
    closeTaskModal();
    renderPqrsOficinaInbox();
    if(typeof renderActividades==='function')renderActividades();
    notif('📬 Pasó a «Por notificar» · vence '+fmtF(notifVence),'ok');
  }
  return true;
}
...
function openPqrsNotificarOficioModal(expId){
  ...
  const modal=ov?ov.querySelector('.task-modal'):null;
  if(!ov||!body)return;
  if(tit)tit.textContent='Por notificar — '+expId;
  if(modal)modal.classList.add('task-modal-wide');
  const wf=getPqrsWorkflow(e);
  const canal=wf.canal||PQRS_WF_CANAL.CORREO;
  ...
  if(esVitalNcaPlantilla){
    ...
    if(plant&&(!cuerpoNotif||cuerpoNotif.length<80||/^Oficio\s/i.test(cuerpoNotif)||(typeof _pqrsEsPlantillaRespuesta==='function'&&_pqrsEsPlantillaRespuesta(cuerpoNotif)))){
      cuerpoNotif=plant;
      window._pqrsUltimaPlantillaOficio=plant;
    }
  }
  body.innerHTML=
    ...
    '<button type="button" class="btn bsm bp" onclick="document.getElementById(\'pqrs-notif-oficio-file\').click()">📎 Cargar oficio notificado</button>'+
    '<input type="file" id="pqrs-notif-oficio-file" accept=".pdf,application/pdf" style="display:none">'+
    '<span id="pqrs-notif-oficio-name" class="sst-file-pick-name">Sin archivo seleccionado</span></div></div>'+
    ...
    '<div class="sst-file-pick">'+
    '<button type="button" class="btn bsm bp" onclick="document.getElementById(\'pqrs-notif-soporte\').click()">📎 Seleccionar archivo</button>'+
    '<input type="file" id="pqrs-notif-soporte" accept=".pdf,.png,.jpg,.jpeg,application/pdf,image/*" style="display:none">'+
    '<span id="pqrs-notif-soporte-name" class="sst-file-pick-name">Sin archivo seleccionado</span></div>'+
  ...
  if(sop)sop.addEventListener('change',function(){
    window._pqrsNotifSoporteFile=(this.files&&this.files[0])||null;
    const nm=document.getElementById('pqrs-notif-soporte-name');
    if(nm)nm.textContent=window._pqrsNotifSoporteFile?(window._pqrsNotifSoporteFile.name||'Archivo seleccionado'):'Sin archivo seleccionado';
  });
  const ofiFile=document.getElementById('pqrs-notif-oficio-file');
  if(ofiFile)ofiFile.addEventListener('change',function(){
    window._pqrsNotifOficioFile=(this.files&&this.files[0])||null;
    const nm=document.getElementById('pqrs-notif-oficio-name');
    if(nm)nm.textContent=window._pqrsNotifOficioFile?(window._pqrsNotifOficioFile.name||'Archivo seleccionado'):'Sin archivo seleccionado';
  });
}
function pqrsNotifSetCanal(val){
  const hid=document.getElementById('pqrs-notif-canal');
  if(hid)hid.value=val||'';
  document.querySelectorAll('#pqrs-notif-canal-btns .canal-resp-btn').forEach(function(b){
    b.classList.toggle('on',b.getAttribute('data-val')===val);
  });
  const correo=document.getElementById('pqrs-notif-correo-box');
  const otro=document.getElementById('pqrs-notif-otro-box');
  ...
  if(wrap){
    ...
    const eN=typeof getExpById==='function'?getExpById(ctx.expId):null;
    const wfN=eN&&typeof getPqrsWorkflow==='function'?getPqrsWorkflow(eN):{};
    const prev=String((document.getElementById('pqrs-notif-por-sel')||{}).value||wfN.notificar_por||'').trim();
    wrap.outerHTML=_pqrsOpcionesNotificadorHtml(eN,wfN,prev,{modo:'director',canal:val});
  }
}
...
async function pqrsConfirmarNotificacionOficio(expId){
  ...
  if(fileOficio){
    try{
      ...
      const nombreCarpeta=(e._qd_nombre||e._pn_nombre||expId);
      const resOf=await driveUploadInstitutional(fileOficio,'oficio-notificado-'+fileOficio.name,fileOficio.type||'application/pdf','respuesta_aprobada',expId,nombreCarpeta,e._fecha||e._fecha_solicitud||'',{expediente:e,uploadTarget:'respuesta'});
      ...
      const nuevo={
        nombre:'oficio-notificado-'+fileOficio.name,
      ...
      setPqrsWorkflow(e,{documentos:otros.concat(anexosKeep).concat([nuevo])});
      window._pqrsNotifOficioFile=null;
    }catch(errOf){
      notif('No se pudo subir el oficio notificado: '+String(errOf.message||errOf).slice(0,90),'err');
      if(btn){btn.disabled=false;btn.textContent='✅ Confirmar notificación';}
      return;
    }
  }
  // Refrescar wf tras posible carga de oficio notificado
  ...
  if(canal===PQRS_WF_CANAL.CORREO||canal==='correo'){
    ...
    const asunto=String((document.getElementById('pqrs-notif-asunto')||{}).value||'').trim();
    const cuerpo=String((document.getElementById('pqrs-notif-cuerpo')||{}).value||'').trim();
    const destinos=toRaw.split(/[,;]+/).map(s=>s.trim().toLowerCase()).filter(s=>s.includes('@'));
    if(!destinos.length){notif('Verifique el correo de destino','err');if(btn){btn.disabled=false;btn.textContent='✅ Confirmar notificación';}return;}
    if(!cuerpo){notif('Indique el mensaje','err');if(btn){btn.disabled=false;btn.textContent='✅ Confirmar notificación';}return;}
    setPqrsWorkflow(e,{email_to:toRaw,email_cc:ccRaw,email_bcc:bccRaw,email_subject:asunto,cuerpo:cuerpo,canal:PQRS_WF_CANAL.CORREO});
    // Solo envía si hay token de oficina (nunca desde Gmail personal — VITAL usa cuenta del encargado NCA)
    if(tokOfi){
      try{
        if(btn)btn.textContent='Descargando anexos…';
        const docsAdj=wfActual.documentos||[];
        const adjuntos=typeof pqrsPrepararAdjuntosNotificacionCorreo==='function'
          ?await pqrsPrepararAdjuntosNotificacionCorreo(docsAdj)
          :[];
        ...
        renderPqrsOficinaInbox();
        if(typeof renderActividades==='function')renderActividades();
        notif('📧 Correo enviado desde la cuenta del encargado NCA'+(nAdj?' con '+nAdj+' adjunto(s)':'')+' — PQRSD atendida','ok');
        return;
      }catch(err){
...
async function ncaAprobarRevisionFinalNotif(expId){
  ...
  if(!Array.isArray(e._pqrs_historial))e._pqrs_historial=[];
  e._pqrs_historial.push({tipo:'revision_final_aprobada',fecha:hoy(),nota:'Encargado aprobó notificación final',por:responsableActivo||'NCA'});
  persistExpedienteGranular(e);
  closeTaskModal();
  renderPqrsOficinaInbox();
...
// Compat: mantener vitalFinalizarGestion / vitalSubirPdfFirmado apuntando al flujo nuevo
async function vitalSubirPdfFirmado(expId){return pqrsDirectorConfirmarFirmado(expId);}
async function vitalFinalizarGestion(expId){return openPqrsNotificarOficioModal(expId);}
...
function pqrsCorreoHtmlRespuesta(e,cuerpo,documentos){
  ...
  if(linksHtml)h+='<hr><p><strong>Documentos de respuesta</strong> (también adjuntos a este correo cuando fue posible):</p>'+linksHtml;
 ...
 */
async function pqrsPrepararAdjuntosNotificacionCorreo(documentos){
  const docs=(Array.isArray(documentos)?documentos:[]).filter(function(d){
    return d&&(d.driveLink||d.fileId||d.driveFileId||d.previewLink)&&d.tipo!=='notificacion_soporte';
  });
  const anexos=docs.filter(function(d){return typeof _pqrsDocEsAnexoRespuesta==='function'&&_pqrsDocEsAnexoRespuesta(d);});
  const oficios=docs.filter(function(d){
    if(typeof _pqrsDocEsAnexoRespuesta==='function'&&_pqrsDocEsAnexoRespuesta(d))return false;
    return d.tipo==='oficio_firmado'||d.tipo==='drive'||!d.tipo;
  });
  // Preferir oficio_firmado; si no hay, incluir oficios no-anexo
  const oficFirm=oficios.filter(function(d){return d.tipo==='oficio_firmado';});
  const oficPick=oficFirm.length?oficFirm:oficios.slice(0,1);
  const target=oficPick.concat(anexos);
  const seen=new Set();
  const files=[];
  for(let i=0;i<target.length;i++){
    const d=target[i];
    const key=String(d.fileId||d.driveFileId||d.driveLink||d.nombre||i).trim().toLowerCase();
    if(seen.has(key))continue;
    seen.add(key);
    try{
      const f=await pqrsDriveDocToFile(d);
      if(f)files.push(f);
    }catch(err){
...
function confirmarEnvioRespuestaEmailPqrs(e,ciudEmail,cuerpo,documentos){
  ...
  body.innerHTML=
    ...
    '<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:600;margin-bottom:4px">Documentos (se adjuntan al correo + links):</div>'+docsHtml+'</div>'+
...
async function enviarCorreoRespuestaPqrs(expId){
  ...
  if(btn){btn.disabled=true;btn.textContent='Enviando…';}
  if(statusEl)statusEl.textContent='⬆ Preparando adjuntos y enviando…';

  const docs=window._pqrsMailDocs||[];
  const bodyHtml=pqrsCorreoHtmlRespuesta(e,cuerpo,docs);
  const ccRaw=String((document.getElementById('pqrs-mail-cc')||{}).value||'').trim();
  const bccRaw=String((document.getElementById('pqrs-mail-bcc')||{}).value||'').trim();
