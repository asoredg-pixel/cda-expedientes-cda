...
function toggleSecAnonimo(){
  ...
  ['sec-pn-nombre','sec-pn-identificacion','sec-pn-correo','sec-pn-telefono','sec-pj-empresa','sec-pj-nit','sec-pj-correo','sec-pj-telefono','sec-pj-ofi-nombre','sec-pj-ofi-identificacion','sec-pj-ofi-correo','sec-pj-ofi-telefono'].forEach(id=>{
    if(typeof setUiFieldDisabled==='function')setUiFieldDisabled(id,anon,anon);
    else{
...
function limpiarFormSecretaria(){
  ...
  ['sec-exp','sec-asunto','sec-detalle','sec-fecha-termino','sec-fecha-solicitud','sec-pn-nombre','sec-pn-identificacion','sec-pn-correo','sec-pn-telefono','sec-pj-empresa','sec-pj-nit','sec-pj-correo','sec-pj-telefono','sec-pj-ofi-nombre','sec-pj-ofi-identificacion','sec-pj-ofi-correo','sec-pj-ofi-telefono','sec-anon-correo','sec-anon-tel'].forEach(id=>{
    if(typeof clearUiField==='function')clearUiField(id);
    else{const el=document.getElementById(id);if(el)el.value='';}
  });
...
async function guardarPqrsSecretaria(modo){
  ...
  if(interna){
    ...
    if(tipoPersonaRaw==='juridica'){
      ...
      pjFields._pj_nit=typeof formatNitDisplay==='function'?formatNitDisplay((document.getElementById('sec-pj-nit')||{}).value||''):String((document.getElementById('sec-pj-nit')||{}).value||'').trim();
      ...
      const ofiIdRaw=String((document.getElementById('sec-pj-ofi-identificacion')||{}).value||'').trim();
      const ofiId=typeof formatIdentDisplay==='function'?formatIdentDisplay(ofiIdRaw):ofiIdRaw;
      ...
      ident=typeof formatIdentDisplay==='function'?formatIdentDisplay((document.getElementById('sec-pn-identificacion')||{}).value||''):String((document.getElementById('sec-pn-identificacion')||{}).value||'').trim();
...
function htmlPqrsOficinaInteresado(e){
  ...
  if(e._tipo_persona==='juridica'){
    ...
    if(e._pj_nit)h+='<br>NIT: '+escAttr(typeof formatNitDisplay==='function'?formatNitDisplay(e._pj_nit):e._pj_nit);
...
function getPqrsOficinaList(oficinaId,filtro){
  ...
  if(filtro==='por_firmar'){
    ...
    let listF=exps.filter(e=>{
      ...
      // Director y oficinas: firmados físicos siguen visibles en «Por firmar» (✍️✓).
      // La paleta Firmados (solo Director/NCA) es vista adicional.
      return true;
    }).map(normalizePqrsOficinaFields);
    // Director: todas; Secretaría: consolidado; NCA/VITAL: solo Guaviare; oficina DEGUV: la suya
    if(!esDir){
      listF=listF.filter(function(e){
        return typeof pqrsVisiblePaletaPorFirmar==='function'?pqrsVisiblePaletaPorFirmar(e,null):true;
      });
    }
    ...
    if(typeof getTramiteFirmaRowsParaPaletaDirector==='function'){
      ...
      if(typeof filterTramiteFirmaRowsPorOficina==='function')
        tramRows=filterTramiteFirmaRowsPorOficina(tramRows,oficinaId||getPqrsOficinaActiva(),esDir,{paletaPorFirmar:true});
      else if(!esDir){
...
function pqrsDirectorPorFirmarAccionesHtml(e){
  ...
  if(e&&e._tramite_firma_task&&e._taskId){
    ...
    return '<button type="button" class="btn bsm bic act-ico" onclick="event.stopPropagation();openDirectorRevisarPorFirmar(\''+id+'\',\''+tid+'\')" title="Revisar documento a firmar">🧐</button> '+
      '<button type="button" class="btn bsm bic act-ico" onclick="event.stopPropagation();openDirectorCargarFirmado(\''+id+'\',\''+tid+'\')" title="Cargar documento firmado">📤</button> '+
...
/**
 * Acciones Director en «Firmados»: solo 🔍 (ver documento firmado/escaneado/notificado/soporte correo).
 * Documento/comunicado de oficina: mismo rail que Respondidas de las oficinas.
 */
function pqrsDirectorFirmadosAccionesHtml(e){
  ...
  const esOfiDoc=!!(e&&(e._oficina_firma||e._oficina_doc_respondida)
    ||(t&&typeof taskEsDocumentoComunicadoOficina==='function'&&taskEsDocumentoComunicadoOficina(t))
    ||(t&&t.origen==='oficina_firma'));
  if(esOfiDoc&&tid){
    return '<button type="button" class="btn bsm bic act-ico" onclick="event.stopPropagation();openOficinaDocRespondida(\''+escAttr(e._exp)+'\',\''+escAttr(e._taskId)+'\')" title="Ver documento / comunicado">🔍</button>';
  }
  const verJs=tid
    ?('openDirectorVerDocumentoFirmados(\''+id+'\',\''+tid+'\')')
    :('openDirectorVerDocumentoFirmados(\''+id+'\')');
...
/** Secretaría en «Por firmar»: chat · entregar (solo si está a cargo) · lupa documento por firmar. */
function pqrsSecretariaPorFirmarAccionesHtml(e){
  const id=jsStr(e&&e._exp);
  if(!id)return'';
  let t=typeof getPqrsAtencionTask==='function'?getPqrsAtencionTask(e):null;
  if(!t&&typeof getPqrsTaskActiva==='function')t=getPqrsTaskActiva(e);
  const tid=t?jsStr(t.id):'';
  let h='<span class="sst-act-toolbar">';
  if(t&&tid&&typeof taskChatBtnHtml==='function')h+=taskChatBtnHtml(e._exp,t.id,t);
  if(String(e._pqrs_oficina||'').trim()==='secretaria'&&typeof puedeMarcarPqrsRespondida==='function'&&puedeMarcarPqrsRespondida(e))
    h+='<button type="button" class="btn bsm bic act-ico act-ico-btn" title="Entregar respuesta" onclick="event.stopPropagation();openPqrsEntregaDirecta(\''+id+'\')">📤</button> ';
  if(tid){
    const eid=escAttr(e._exp),tidEsc=escAttr(tid);
    if(typeof openActPorFirmarDocs==='function')
      h+='<button type="button" class="btn bsm bic act-ico" title="Ver documento por firmar" onclick="event.stopPropagation();openActPorFirmarDocs(\''+eid+'\',\''+tidEsc+'\')">🔍</button>';
    else if(typeof openTaskVerDocumentoResp==='function')
      h+='<button type="button" class="btn bsm bic act-ico" title="Ver documento por firmar" onclick="event.stopPropagation();openTaskVerDocumentoResp(\''+eid+'\',\''+tidEsc+'\',{soloAprobados:true,porFirmarVista:true})">🔍</button>';
  }
  h+='</span>';
  return h;
}
function pqrsSecretariaPorFirmarDocAccionesHtml(e,t){
  ...
  let h='<span class="sst-act-toolbar">';
  if(typeof tramitePuedeGestionarComoOficina==='function'&&tramitePuedeGestionarComoOficina(t)){
    if(typeof actPuedeCargarFirmadoPorFirmar==='function'?actPuedeCargarFirmadoPorFirmar():true){
      if(typeof actPorFirmarCargarBtnHtml==='function')
        ...
        h+='<button type="button" class="btn bsm bic act-ico act-ico-btn" title="Entregar respuesta" onclick="event.stopPropagation();openCargarFirmadoPorFirmar(\''+eid+'\',\''+tid+'\')">📤</button> ';
    }
  }
  if(typeof openActPorFirmarDocs==='function')
    h+='<button type="button" class="btn bsm bic act-ico" title="Ver documento por firmar" onclick="event.stopPropagation();openActPorFirmarDocs(\''+eid+'\',\''+tid+'\')">🔍</button>';
  else if(typeof openTaskVerDocumentoResp==='function')
    h+='<button type="button" class="btn bsm bic act-ico" title="Ver documento por firmar" onclick="event.stopPropagation();openTaskVerDocumentoResp(\''+eid+'\',\''+tid+'\',{soloAprobados:true,porFirmarVista:true})">🔍</button>';
  h+='</span>';
  return h;
}
window.pqrsSecretariaPorFirmarAccionesHtml=pqrsSecretariaPorFirmarAccionesHtml;
window.pqrsSecretariaPorFirmarDocAccionesHtml=pqrsSecretariaPorFirmarDocAccionesHtml;
...
function pqrsAccionesTablaHtml(e){
  ...
  const esSecCons=typeof pqrsSesionConsolidaPorFirmarTodasOficinas==='function'&&pqrsSesionConsolidaPorFirmarTodasOficinas();
  if(esSecCons&&filtroOfi==='por_firmar'){
    if(e&&e._tramite_firma_task&&e._taskId){
      const tSec=typeof getTaskAny==='function'?getTaskAny(e._exp,e._taskId):null;
      return pqrsSecretariaPorFirmarDocAccionesHtml(e,tSec);
    }
    return pqrsSecretariaPorFirmarAccionesHtml(e);
  }
  // Oficinas + NCA: «Por ejecutar» / «Vencidas» / «Por firmar» = mismo toolbar
  ...
  // Oficinas (no Director): en «Por firmar» mismas acciones que Por ejecutar
  if(!esDir&&esOfiToolbar&&filtroOfi==='por_firmar'&&!e._tramite_firma_task){
    const fasePf=typeof pqrsWorkflowFase==='function'?pqrsWorkflowFase(e):'';
  ...
  // Documento/comunicado ya respondido (mensaje simple u oficio notificado) — oficinas y Director
  if(e&&e._tramite_firma_task&&e._taskId&&(e._oficina_firma||e._oficina_doc_respondida||filtroOfi==='firmados'||filtroOfi==='cerr')){
    const tDoc=typeof getTaskAny==='function'?getTaskAny(e._exp,e._taskId):null;
    const esOfiDoc=!!(e._oficina_firma||e._oficina_doc_respondida
      ||(tDoc&&typeof taskEsDocumentoComunicadoOficina==='function'&&taskEsDocumentoComunicadoOficina(tDoc))
      ||(tDoc&&tDoc.origen==='oficina_firma'));
    const cerradaDoc=!!(e._oficina_doc_respondida
      ||(tDoc&&(String(tDoc.estado||'')==='Atendida'
        ||(typeof taskFirmaEsNotificada==='function'&&taskFirmaEsNotificada(tDoc))
        ||['cerrada_atendida','cerrada'].indexOf(String((tDoc.firmaWf&&tDoc.firmaWf.fase)||''))>=0
        ||(typeof PQRS_WF!=='undefined'&&String((tDoc.firmaWf&&tDoc.firmaWf.fase)||'')===PQRS_WF.CERRADA))));
    if(esOfiDoc&&(filtroOfi==='cerr'||(filtroOfi==='firmados'&&(cerradaDoc||esDir)))){
...
function matchPqrsOfiSearch(e,q){
  if(!q)return true;
  const ql=String(q).toLowerCase().trim();
  if(!ql)return true;
  if(typeof matchS==='function'&&matchS(e,ql))return true;
  const blob=[e._exp,e.f_f1,e._pqrs_detalle,e._tipo_solicitud,e._pn_nombre,e._pn_identificacion,
    e._pj_empresa,e._pj_nit,e._oficina_firma,e._taskId,e.actividad].filter(Boolean).join(' ').toLowerCase();
...
window.matchPqrsOfiSearch=matchPqrsOfiSearch;
var PQRS_OFI_PAGE=30;
function onPqrsOfiSearchInput(){
  window._pqrsOfiShown=PQRS_OFI_PAGE;
  renderPqrsOficinaInbox();
}
window.onPqrsOfiSearchInput=onPqrsOfiSearchInput;
function pqrsOfiVerMas(){
  window._pqrsOfiShown=(window._pqrsOfiShown||PQRS_OFI_PAGE)+PQRS_OFI_PAGE;
  renderPqrsOficinaInbox();
}
window.pqrsOfiVerMas=pqrsOfiVerMas;
function pqrsOfiFiltroLabel(filtro){
  const m={
    pend:'Por ejecutar',atras:'Vencidas',revision:'Por revisar',para_firma:'Por imprimir',
    por_firmar:'Por firmar',firmados:'Firmados',por_notificar:'Por notificar',cerr:'Respondidas',
    por_trasladar:'Por trasladar',all:'Todas'
  };
  return m[filtro]||filtro||'Filtro';
}
function exportarPqrsOficinaExcel(){
  const list=window._pqrsOfiExportList||[];
  if(!list.length){notif('Sin registros para exportar en este filtro','err');return;}
  if(typeof exportarTablaExcel!=='function'){notif('Exportación no disponible','err');return;}
  const filtro=window._pqrsOfiFiltro||'pend';
  const hdr=['Expediente','Tipo','Asunto','Radicación','Estado','Oficina','Interesado / NIT'];
  const rows=list.map(function(e){
    const tipo=e._tramite_firma_task
      ?((e._oficina_firma||e._tipo_solicitud==='Oficio oficina')?'Oficio':'Trámite')
      :(e._tipo_solicitud||'PQRSD');
    let est='';
    if(e._tramite_firma_task&&typeof getTaskAny==='function'&&typeof taskFirmaEstadoUi==='function'){
      const t=getTaskAny(e._exp,e._taskId);
      const ui=t?taskFirmaEstadoUi(t):null;
      est=ui?(ui.lbl+(ui.sub?' · '+ui.sub:'')):'';
    }else if(typeof pqrsWorkflowFase==='function'){
      est=String(pqrsWorkflowFase(e)||'');
    }
    const interesado=[e._pn_nombre,e._pj_empresa,e._pj_nit,e._pn_identificacion].filter(Boolean).join(' · ');
    return [
      e._exp||'',
      tipo,
      e.f_f1||e._pqrs_detalle||'',
      typeof fmtF==='function'?fmtF(e._fecha): (e._fecha||''),
      est,
      e._pqrs_oficina||e._oficina_firma||'',
      interesado
    ];
  });
  const suf=String(pqrsOfiFiltroLabel(filtro)||filtro).replace(/\W+/g,'-').toLowerCase();
  exportarTablaExcel([{title:pqrsOfiFiltroLabel(filtro),hdr,rows}],'pqrs-ofi-'+suf+'-'+(typeof hoy==='function'?hoy():Date.now()));
  notif('Excel descargado ('+list.length+' fila'+(list.length===1?'':'s')+')','ok');
}
window.exportarPqrsOficinaExcel=exportarPqrsOficinaExcel;
function renderPqrsOficinaInbox(){
  ...
  if((filtro==='firmados'||filtro==='por_notificar')
  ...
  let list=getPqrsOficinaList(getPqrsOficinaActiva(),filtro);
  const qOfi=(document.getElementById('s-pqrs-ofi')?document.getElementById('s-pqrs-ofi').value:'').trim();
  if(qOfi)list=(list||[]).filter(function(e){return matchPqrsOfiSearch(e,qOfi);});
  window._pqrsOfiExportList=list||[];
  if(window._pqrsOfiShown==null||window._pqrsOfiShown<PQRS_OFI_PAGE)window._pqrsOfiShown=PQRS_OFI_PAGE;
  const listFull=list||[];
  const shown=Math.min(window._pqrsOfiShown,listFull.length);
  list=listFull.slice(0,shown);
  const moreBtn=document.getElementById('pqrs-ofi-more');
  if(moreBtn){
    const rest=listFull.length-shown;
    if(rest>0){
      moreBtn.style.display='';
      moreBtn.textContent='Cargar más ('+rest+' restantes)';
  ...
  if(mets){
    ...
    const showFirmadosCard=(typeof pqrsPuedeVerPaletaFirmados==='function'?pqrsPuedeVerPaletaFirmados():esDirMets)
      &&!(typeof pqrsOficinaSinPaletasFirmadosNotif==='function'&&pqrsOficinaSinPaletasFirmadosNotif());
    const showPorNotificarCard=!(typeof pqrsOficinaSinPaletasFirmadosNotif==='function'&&pqrsOficinaSinPaletasFirmadosNotif())
      &&!esDirMets;
    ...
    const tramPorFirmarRows=(typeof getTramiteFirmaRowsParaPaletaDirector==='function'
      ?(typeof filterTramiteFirmaRowsPorOficina==='function'
        ?filterTramiteFirmaRowsPorOficina(getTramiteFirmaRowsParaPaletaDirector('por_firmar')||[],ofiActMets,esDirMets,{paletaPorFirmar:true})
    ...
    const tramPorNotifRows=(typeof getTramiteFirmaRowsParaPaletaDirector==='function'
      ...
      :[]);
    const porFirmar=showPorFirmarCard?getPqrsOficinaList(getPqrsOficinaActiva(),'por_firmar').length:0;
    ...
    mets.innerHTML=
      ...
      (showPorNotificarCard&&(porNotif||showPorFirmarCard)?pqrsMetCard('por_notificar','border-left:3px solid var(--bl)','<div class="v" style="color:var(--bl)">'+porNotif+'</div><div class="l">Por notificar</div>','var(--bl)'):'')+
      pqrsMetCard('cerr','border-left:3px solid var(--gn)','<div class="v" style="color:var(--gn)">'+cerr+'</div><div class="l">Respondidas</div>','var(--gn)');
  }
  if(!list.length){
    const vacioMsg=qOfi
      ?('Sin resultados para «'+escAttr(qOfi)+'».')
      :(esPendTrasl?'No hay PQRSD pendientes de traslado en este filtro de fechas.':'No hay PQRSD en este filtro.');
  ...
  if(!sel||!listFull.some(function(e){
    const k=String(e._exp||'').trim()+(e._tramite_firma_task&&e._taskId?('#'+e._taskId):'');
    return k===sel||String(e._exp||'').trim()===sel;
  }))window._pqrsOfiSelExp=String(list[0]._exp||'').trim()+(list[0]._tramite_firma_task&&list[0]._taskId?('#'+list[0]._taskId):'');
  const esSecConsTab=typeof pqrsSesionConsolidaPorFirmarTodasOficinas==='function'&&pqrsSesionConsolidaPorFirmarTodasOficinas();
  tb.innerHTML=list.map(e=>{
    const asuntoBase=e.f_f1||e._pqrs_detalle||'—';
    const tRow=e._taskId&&typeof getTaskAny==='function'?getTaskAny(e._exp,e._taskId):null;
    const ofiPfLbl=(esSecConsTab&&filtro==='por_firmar')
      ?(typeof labelOficina==='function'?labelOficina(typeof pqrsOficinaCtxPorFirmar==='function'?pqrsOficinaCtxPorFirmar(e,tRow):String(e._pqrs_oficina||'')):String(e._pqrs_oficina||'—'))
      :'';
    const asunto=ofiPfLbl
      ?('<span style="display:block;font-size:11px;color:var(--tx2);margin-bottom:2px">Enviado por: <strong>'+escAttr(ofiPfLbl)+'</strong></span>'+escAttr(asuntoBase))
      :escAttr(asuntoBase);
    // Estado unificado (✓ Revisada · X Firma) — igual para Director / VITAL / encargado / oficina
    const wfBadge=e._tramite_firma_task
      ?(function(){
        ...
        const esOfiDoc=!!(e._oficina_firma||e._oficina_doc_respondida
          ||(t&&typeof taskEsDocumentoComunicadoOficina==='function'&&taskEsDocumentoComunicadoOficina(t))
          ||(t&&t.origen==='oficina_firma'));
        const f=t?String((typeof taskFirmaFase==='function'?taskFirmaFase(t):((t.firmaWf&&t.firmaWf.fase)||''))||'').trim():'';
        const cerrada=!!(e._oficina_doc_respondida
          ||(t&&(String(t.estado||'')==='Atendida'
            ||(typeof taskFirmaEsNotificada==='function'&&taskFirmaEsNotificada(t))
            ||f==='cerrada_atendida'||f==='cerrada'
            ||(typeof PQRS_WF!=='undefined'&&f===PQRS_WF.CERRADA))));
        // Director Firmados / Respondidas: Documento/comunicado cerrado → ✓ Atendida
        if(esOfiDoc&&cerrada)
          return'<span class="bdg" style="background:var(--gnl);color:var(--gn)">✓ Atendida</span>';
        if(t&&typeof taskFirmaEstadoUi==='function'){
          const ui=taskFirmaEstadoUi(t);
          if(ui&&ui.lbl){
            if(ui.sub){
              const subBg=ui.subBg||'#fef9c3',subFg=ui.subFg||'#a16207';
              return '<span class="act-est-dual"><span class="bdg" style="background:'+(ui.bg||'var(--gnl)')+';color:'+(ui.fg||'var(--gn)')+'">'+escAttr(ui.lbl)+'</span>'+
                '<span class="bdg act-est-sub-badge" style="background:'+subBg+';color:'+subFg+';border:1px solid #fde68a">'+escAttr(ui.sub)+'</span></span>';
            }
    ...
    // En firma/notif: solo badges de flujo (como trámite). Sin «En trámite» ni 🌐.
    const hideExpEstadoExtra=!!e._tramite_firma_task
    ...
    // Excepto Documento/comunicado cerrado: clic abre el mismo visor que 🔍
    const esDocRespondida=!!(e._oficina_doc_respondida||(filtro==='cerr'&&e._tramite_firma_task&&e._oficina_firma)
      ||(filtro==='firmados'&&e._tramite_firma_task&&e._oficina_firma&&e._oficina_doc_respondida));
    const esFilaFirma=(filtro==='por_firmar'||filtro==='firmados')&&!esDocRespondida;
...
function openEditPqrsSecretariaModal(expId){
  ...
  body.innerHTML='<div style="font-size:12px;color:var(--tx2);margin-bottom:10px">Puede modificar todos los datos de la solicitud. Si cambia la oficina destino se registrará un traslado.</div>'+
    ...
    '<div class="fld"><label>Identificación</label><input type="text" id="pqrs-edit-pn-identificacion" value="'+escAttr(typeof formatIdentDisplay==='function'?formatIdentDisplay(e._pn_identificacion||e._qd_identificacion||''):(e._pn_identificacion||e._qd_identificacion||''))+'" inputmode="numeric" oninput="onIdentMilesInput(this)"></div>'+
    ...
    '<div class="fld"><label>NIT</label>'+(typeof htmlNitConDvField==='function'?htmlNitConDvField('pqrs-edit-pj-nit',{value:e._pj_nit||'',placeholder:'NIT'}):('<input type="text" id="pqrs-edit-pj-nit" value="'+escAttr(e._pj_nit||'')+'">'))+'</div>'+
    ...
    '<div class="fld"><label>Identificación oficial</label><input type="text" id="pqrs-edit-pj-ofi-identificacion" value="'+escAttr(typeof formatIdentDisplay==='function'?formatIdentDisplay(e._qd_identificacion&&e._qd_identificacion!==e._pj_nit?e._qd_identificacion:''):(e._qd_identificacion&&e._qd_identificacion!==e._pj_nit?e._qd_identificacion:''))+'" inputmode="numeric" oninput="onIdentMilesInput(this)"></div>'+
...
async function submitEditPqrsSecretaria(expId){
  ...
  if(anon){
    ...
    e._pj_nit=typeof formatNitDisplay==='function'?formatNitDisplay((document.getElementById('pqrs-edit-pj-nit')||{}).value||''):String((document.getElementById('pqrs-edit-pj-nit')||{}).value||'').trim();
    ...
    e._qd_identificacion=typeof formatIdentDisplay==='function'?formatIdentDisplay((document.getElementById('pqrs-edit-pj-ofi-identificacion')||{}).value||''):String((document.getElementById('pqrs-edit-pj-ofi-identificacion')||{}).value||'').trim();
    ...
    e._pn_identificacion=typeof formatIdentDisplay==='function'?formatIdentDisplay((document.getElementById('pqrs-edit-pn-identificacion')||{}).value||''):String((document.getElementById('pqrs-edit-pn-identificacion')||{}).value||'').trim();
